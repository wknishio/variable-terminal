...
public class IndexDecoder extends IndexBase {
	...
	public IndexDecoder(SeekableInputStream in, StreamFlags streamFooterFlags, long streamPadding, int memoryLimit)
		...
		try {
			...
			if (count > Integer.MAX_VALUE)
				throw new UnsupportedOptionsException("XZ Index has over " + Integer.MAX_VALUE + " Records");
	...
	public void setOffsets(IndexDecoder prev) {
		...
		compressedOffset = prev.compressedOffset + prev.getStreamSize() + prev.streamPadding;
	...
	public boolean hasUncompressedOffset(long pos) {
		return pos >= uncompressedOffset && pos < uncompressedOffset + uncompressedSum;
	}

	public boolean hasRecord(int blockNumber) {
		return blockNumber >= recordOffset && blockNumber < recordOffset + recordCount;
	}
	...
	public void setBlockInfo(BlockInfo info, int blockNumber) {
		...
		info.compressedOffset += compressedOffset + DecoderUtil.STREAM_HEADER_SIZE;
