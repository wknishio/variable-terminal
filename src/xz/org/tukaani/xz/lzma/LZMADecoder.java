...
public final class LZMADecoder extends LZMACoder {
	...
	/**
	 * Returns true if LZMA end marker was detected. It is encoded as the maximum
	 * match distance which with signed ints becomes -1. This function is needed
	 * only for LZMA1. LZMA2 doesn't use the end marker in the LZMA layer.
	 */
	...
	private class LengthDecoder extends LengthCoder {
		int decode(int posState) throws IOException {
			...
			if (rc.decodeBit(choice, 1) == 0)
				return rc.decodeBitTree(mid[posState]) + MATCH_LEN_MIN + LOW_SYMBOLS;
