...
public abstract class LZMAEncoder extends LZMACoder {
	 ...
	 * <p>
	 * A compressed LZMA2 chunk can hold 2 MiB of uncompressed data. A single LZMA
	 * symbol may indicate up to MATCH_LEN_MAX bytes of data, so the LZMA2 chunk is
	 * considered full when there is less space than MATCH_LEN_MAX bytes.
	 */
	 ...
	 * <p>
	 * The maximum compressed size of a LZMA2 chunk is 64 KiB. A single LZMA symbol
	 * might use 20 bytes of space even though it usually takes just one byte or so.
	 * Two more bytes are needed for LZMA2 uncompressed chunks (see
	 * LZMA2OutputStream.writeChunk). Leave a little safety margin and use 26 bytes.
	 */
	...
	public static LZMAEncoder getInstance(RangeEncoder rc, int lc, int lp, int pb, int mode, int dictSize,
			int extraSizeBefore, int niceLen, int mf, int depthLimit) {
		switch (mode) {
		case MODE_FAST:
	...
	/**
	 * Gets an integer [0, 63] matching the highest two bits of an integer. This is
	 * like bit scan reverse (BSR) on x86 except that this also cares about the
	 * second highest bit.
	 */
	 ...
	 * <p>
	 * There are three types of symbols: literal (a single byte), repeated match,
	 * and normal match. The symbol is indicated by the return value and by the
	 * variable <code>back</code>.
	 * <p>
	 * Literal: <code>back == -1</code> and return value is <code>1</code>. The
	 * literal itself needs to be read from <code>lz</code> separately.
	 * <p>
	 * Repeated match: <code>back</code> is in the range [0, 3] and the return value
	 * is the length of the repeated match.
	 * <p>
	 * Normal match: <code>back - REPS<code> (<code>back - 4</code>) is the distance
	 * of the match and the return value is the length of the match.
	 */
	...
	int getShortRepPrice(int anyRepPrice, State state, int posState) {
		return anyRepPrice + RangeEncoder.getBitPrice(isRep0[state.get()], 0)
	...
	int getLongRepPrice(int anyRepPrice, int rep, State state, int posState) {
		...
		if (rep == 0) {
			price += RangeEncoder.getBitPrice(isRep0[state.get()], 0)
					+ RangeEncoder.getBitPrice(isRep0Long[state.get()][posState], 1);
		} else {
			...
			if (rep == 1)
				...
				price += RangeEncoder.getBitPrice(isRep1[state.get()], 1)
						+ RangeEncoder.getBitPrice(isRep2[state.get()], rep - 2);
		}
	 ...
	 * prices. The updating is skipped for performance reasons if the tables haven't
	 * changed much since the previous update.
	 */
	void updatePrices() {
		if (distPriceCount <= 0)
			updateDistPrices();
	...
	class LiteralEncoder extends LiteralCoder {
		...
		int getPrice(int curByte, int matchByte, int prevByte, int pos, State state) {
			...
			price += state.isLiteral() ? subencoders[i].getNormalPrice(curByte)
	...
	class LengthEncoder extends LengthCoder {
		/**
		 * The prices are updated after at least <code>PRICE_UPDATE_INTERVAL</code> many
		 * lengths have been encoded with the same posState.
		 */
		...
		private void updatePrices(int posState) {
			...
			for (; i < LOW_SYMBOLS + MID_SYMBOLS; ++i)
				prices[posState][i] = choice0Price + choice1Price
						+ RangeEncoder.getBitTreePrice(mid[posState], i - LOW_SYMBOLS);

			choice1Price = RangeEncoder.getBitPrice(choice[1], 1);

			for (; i < prices[posState].length; ++i)
				prices[posState][i] = choice0Price + choice1Price
						+ RangeEncoder.getBitTreePrice(high, i - LOW_SYMBOLS - MID_SYMBOLS);
		}
	}
}
