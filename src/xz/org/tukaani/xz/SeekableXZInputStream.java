...
public class SeekableXZInputStream extends SeekableInputStream {
	 ...
	 * <code>memoryLimit + indexMemoryUsage</code> equals the original memory usage
	 * limit that was passed to the constructor.
	 */
	...
	/**
	 * Temporary buffer for read(). This avoids reallocating memory on every read()
	 * call.
	 */
	 ...
	 *
	 * @param in
	 *            seekable input stream containing one or more XZ Streams; the whole
	 ...
	 *             XZ headers seem valid but they specify options not supported by
	 *             this implementation
	 *
	 * @throws EOFException
	 *             less than 6 bytes of input was available from <code>in</code>, or
	 *             (unlikely) the size of the underlying stream got smaller while
	 ...
	 *
	 * @param memoryLimit
	 *            memory usage limit in kibibytes (KiB) or <code>-1</code> to impose
	 ...
	 *             decoded XZ Indexes would need more memory than allowed by the
	 *             memory usage limit
	 *
	 * @throws EOFException
	 *             less than 6 bytes of input was available from <code>in</code>, or
	 ...
	 * <li>Speeding up decompression. This matters mostly with SHA-256 or with files
	 * that have compressed extremely well. It's recommended that integrity checking
	 * isn't disabled for performance reasons unless the file integrity is verified
	 * externally in some other way.</li>
	 * </ul>
	 * <p>
	 ...
	 *
	 * @param verifyCheck
	 *            if <code>true</code>, the integrity checks will be verified; this
	 *            should almost never be set to <code>false</code>
	 *
	 * @throws XZFormatException
	 *             input is not in the XZ format
	 *
	...
	public SeekableXZInputStream(SeekableInputStream in, int memoryLimit, boolean verifyCheck) throws IOException {
		...
		while (pos > 0) {
			...
			if (blockCount < 0)
				throw new UnsupportedOptionsException("XZ file has over " + Integer.MAX_VALUE + " Blocks");
	...
	/**
	 * Gets the types of integrity checks used in the .xz file. Multiple checks are
	 * possible only if there are multiple concatenated XZ Streams.
	 * <p>
	 * The returned value has a bit set for every check type that is present. For
	 * example, if CRC64 and SHA-256 were used, the return value is
	 ...
	 * needed to locate the XZ Blocks. This is usually useless information but since
	 * it is calculated for memory usage limit anyway, it is nice to make it
	 * available to too.
	 */
	 ...
	 * useful if you want to check that the file doesn't have huge XZ Blocks which
	 * could make seeking to arbitrary offsets very slow. Note that huge Blocks
	 * don't automatically mean that seeking would be slow, for example, seeking to
	 * the beginning of any Block is always fast.
	 */
	...
	/**
	 * Gets the position where the given compressed Block starts in the underlying
	 * .xz file. This information is rarely useful to the users of this class.
	 *
	 * @throws IndexOutOfBoundsException
	 *             if <code>blockNumber&nbsp;&lt;&nbsp;0</code> or
	 *             <code>blockNumber&nbsp;&gt;=&nbsp;getBlockCount()</code>.
	...
	/**
	 * Gets the number of the Block that contains the byte at the given uncompressed
	 * position.
	 *
	 * @throws IndexOutOfBoundsException
	 *             if <code>pos&nbsp;&lt;&nbsp;0</code> or
	 ...
	 *
	 * @return the next decompressed byte, or <code>-1</code> to indicate the end of
	 ...
	 * uncompressed data. Less than <code>len</code> bytes may be read only in the
	 * following situations:
	 * <ul>
	 * <li>The end of the compressed data was reached successfully.</li>
	 * <li>An error is detected after at least one but less than <code>len</code>
	 * bytes have already been successfully decompressed. The next call with
	 * non-zero <code>len</code> will immediately throw the pending exception.</li>
	 ...
	 * The value is returned with an assumption that the compressed input data will
	 * be valid. If the compressed data is corrupt,
	 ...
	 * only stores the new position, so this function itself is always very fast.
	 * The actual seek is done when <code>read</code> is called to read at least one
	 * byte.
	 * <p>
	 ...
	 *
	 * @throws XZIOException
	 *             if <code>pos</code> is negative, or if stream has been closed
	 */
	 ...
	 * @throws XZIOException
	 *             if <code>blockNumber&nbsp;&lt;&nbsp;0</code> or
	 *             <code>blockNumber&nbsp;&gt;=&nbsp;getBlockCount()</code>, or if
	...
	/**
	 * Does the actual seeking. This is also called when <code>read</code> needs a
	 * new Block to decode.
	 */
	...
	private void initBlockDecoder() throws IOException {
		try {
			...
			blockDecoder = null;
			blockDecoder = new BlockInputStream(in, check, verifyCheck, memoryLimit, curBlockInfo.unpaddedSize,
