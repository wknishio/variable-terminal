...
public class LZMAInputStream extends InputStream {
	 ...
	 * <p>
	 * LZMA allows dictionaries up to one byte less than 4 GiB. This implementation
	 * supports only 16 bytes less than 2 GiB. This limitation is due to Java using
	 * signed 32-bit integers for array indexing. The limitation shouldn't matter
	 * much in practice since so huge dictionaries are not normally used.
	 */
	...
	/**
	 * Number of uncompressed bytes left to be decompressed, or -1 if the end marker
	 * is used.
	 */
	...
	/**
	 * Gets approximate decompressor memory requirements as kibibytes for the given
	 * dictionary size and LZMA properties byte (lc, lp, and pb).
	 *
	 * @param dictSize
	...
	/**
	 * Gets approximate decompressor memory requirements as kibibytes for the given
	 * dictionary size, lc, and lp. Note that pb isn't needed.
	 *
	 * @param dictSize
	 ...
	 *            idea to wrap it in <code>BufferedInputStream</code>, see the note
	...
	/**
	 * Creates a new .lzma file format decompressor with an optional memory usage
	 * limit.
	 *
	 * @param in
	 *            input stream from which .lzma data is read; it might be a good
	...
	/**
	 * Creates a new input stream that decompresses raw LZMA data (no .lzma header)
	 * from <code>in</code>.
	 * <p>
	 * The caller needs to know if the "end of payload marker (EOPM)" alias "end of
	 * stream marker (EOS marker)" alias "end marker" present. If the end marker
	 * isn't used, the caller must know the exact uncompressed size of the stream.
	 * <p>
	 * The caller also needs to provide the LZMA properties byte that encodes the
	 * number of literal context bits (lc), literal position bits (lp), and position
	 * bits (pb).
	 * <p>
	 * The dictionary size used when compressing is also needed. Specifying a too
	 * small dictionary size will prevent decompressing the stream. Specifying a too
	 * big dictionary is waste of memory but decompression will work.
	 * <p>
	 * There is no need to specify a dictionary bigger than the uncompressed size of
	 * the data even if a bigger dictionary was used when compressing. If you know
	 * the uncompressed size of the data, this might allow saving some memory.
	 *
	 * @param in
	 *            input stream from which compressed data is read
	 *
	 * @param uncompSize
	 *            uncompressed size of the LZMA stream or -1 if the end marker is
	 *            used in the LZMA stream
	 *
	 * @param propsByte
	 *            LZMA properties byte that has the encoded values for literal
	 *            context bits (lc), literal position bits (lp), and position bits
	 *            (pb)
	 *
	 * @param dictSize
	 ...
	 *
	 * @throws CorruptedInputException
	 *             if <code>propsByte</code> is invalid or the first input byte is
	 *             not 0x00
	 *
	 * @throws UnsupportedOptionsException
	 *             dictionary size or uncompressed size is too big for this
	...
	/**
	 * Creates a new input stream that decompresses raw LZMA data (no .lzma header)
	 * from <code>in</code> optionally with a preset dictionary.
	 *
	 * @param in
	 ...
	 *
	 * @param presetDict
	 *            preset dictionary or <code>null</code> to use no preset dictionary
	 *
	 * @throws CorruptedInputException
	 *             if <code>propsByte</code> is invalid or the first input byte is
	 *             not 0x00
	 ...
	 *             may be thrown by <code>in</code>
	 */
	public LZMAInputStream(InputStream in, long uncompSize, byte propsByte, int dictSize, byte[] presetDict)
			throws IOException {
	 ...
	 *             may be thrown by <code>in</code>
	 */
	public LZMAInputStream(InputStream in, long uncompSize, int lc, int lp, int pb, int dictSize, byte[] presetDict)
			throws IOException {
		initialize(in, uncompSize, lc, lp, pb, dictSize, presetDict);
	}

	private void initialize(InputStream in, long uncompSize, byte propsByte, int dictSize, byte[] presetDict)
			throws IOException {
	...
	private void initialize(InputStream in, long uncompSize, int lc, int lp, int pb, int dictSize, byte[] presetDict)
			throws IOException {
	 ...
	 * inefficient. Wrap it in <code>java.io.BufferedInputStream</code> if you need
	 ...
	 * decompressed, the end of the LZMA stream is reached, or an exception is
	 * thrown.
	 *
	 * @param buf
