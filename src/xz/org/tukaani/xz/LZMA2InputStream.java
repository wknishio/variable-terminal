...
public class LZMA2InputStream extends InputStream {
	 ...
	 * <p>
	 * The LZMA2 algorithm allows dictionaries up to one byte less than 4 GiB. This
	 * implementation supports only 16 bytes less than 2 GiB for raw LZMA2 streams,
	 * and for .xz files the maximum is 1.5 GiB. This limitation is due to Java
	 * using signed 32-bit integers for array indexing. The limitation shouldn't
	 * matter much in practice since so huge dictionaries are not normally used.
	 */
	...
	private final RangeDecoderFromBuffer rc = new RangeDecoderFromBuffer(COMPRESSED_SIZE_MAX);
	 ...
	 * <p>
	 * Specifying a too small dictionary size will prevent decompressing the stream.
	 ...
	 * <p>
	 * This is like <code>LZMA2InputStream(InputStream, int)</code> except that the
	 ...
	 * decompressed, the end of the LZMA2 stream is reached, or an exception is
	 * thrown.
	 *
	 * @param buf
	 ...
	 * <p>
	 * In LZMA2InputStream, the return value will be non-zero when the decompressor
	 * is in the middle of an LZMA2 chunk. The return value will then be the number
	 * of uncompressed bytes remaining from that chunk.
	 *
