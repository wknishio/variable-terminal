...
public class LZMAOutputStream extends FinishableOutputStream {
	...
	private LZMAOutputStream(OutputStream out, LZMA2Options options, boolean useHeader, boolean useEndMarker,
		...
		lzma = LZMAEncoder.getInstance(rc, options.getLc(), options.getLp(), options.getPb(), options.getMode(),
				dictSize, 0, options.getNiceLen(), options.getMatchFinder(), options.getDepthLimit());
	 ...
	 * <p>
	 * If the uncompressed size of the input data is known, it will be stored in the
	 * .lzma header and no end of stream marker will be used. Otherwise the header
	 * will indicate unknown uncompressed size and the end of stream marker will be
	 * used.
	 * <p>
	 ...
	 *
	 * @param options
	 *            LZMA compression options; the same class is used here as is for
	 *            LZMA2
	 *
	 * @param inputSize
	 ...
	 * <p>
	 * Raw LZMA streams can be encoded with or without end of stream marker. When
	 * decompressing the stream, one must know if the end marker was used and tell
	 * it to the decompressor. If the end marker wasn't used, the decompressor will
	 * also need to know the uncompressed size.
	 *
	 * @param out
	 *            output stream to which the compressed data will be written
	 *
	...
	public void write(byte[] buf, int off, int len) throws IOException {
		...
		if (expectedUncompressedSize != -1 && expectedUncompressedSize - currentUncompressedSize < len)
			throw new XZIOException(
					"Expected uncompressed input size (" + expectedUncompressedSize + " bytes) was exceeded");
