...
public final class Blake3 {
    ...
    private static final class Output {
        ...
        private int[] chainingValue() {
            return VTArrays.copyOf(compress(inputChainingValue, blockWords, blockLength, counter, flags), CHAINING_VALUE_INTS);
        }
        ...
        private void rootOutputBytes(final byte[] out, int offset, int length) {
            ...
            while (length > 0) {
                ...
                int wordCounter = 0;
                while (chunkLength > 0) {
                    final int wordLength = Math.min(Integer.SIZE / 8, chunkLength);
    ...
    private static final int BLOCK_INTS = BLOCK_LEN / (Integer.SIZE / 8);
    private static final int KEY_LEN = 32;
    private static final int KEY_INTS = KEY_LEN / (Integer.SIZE / 8);
    ...
    private static void checkBufferArgs(final byte[] buffer, final int offset, final int length) {
        VTObjects.requireNonNull(buffer);
        if (offset < 0) {
    ...
    private static int[] compress(final int[] chainingValue, final int[] blockWords, final int blockLength, final long counter, final int flags) {
        final int[] state = VTArrays.copyOf(chainingValue, BLOCK_INTS);
    ...
    public static Blake3 initKeyDerivationFunction(final byte[] kdfContext) {
        VTObjects.requireNonNull(kdfContext);
    ...
    private static Output parentOutput(final int[] leftChildCV, final int[] rightChildCV, final int[] key, final int flags) {
        final int[] blockWords = VTArrays.copyOf(leftChildCV, BLOCK_INTS);
    ...
    private static int[] unpackInts(final byte[] buf, final int nrInts) {
        ...
        for (int i = 0, off = 0; i < nrInts; i++, off += (Integer.SIZE / 8)) {
