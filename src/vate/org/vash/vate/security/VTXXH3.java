...
public final class VTXXH3
{
  ...
  public static long hash64(byte[] input, int len, long seed) {
    ...
    if (len <= 16) {
       return xxh3_len_0to16(input, len) ^ seed;
    } else if (len <= 128) {
       return xxh3_len_17to128(input, len) ^ seed;
    } else if (len <= 240) {
       return xxh3_len_129to240(input, len) ^ seed;
    } else {
       return xxh3_hashLong(input, len) ^ seed;
    }
 }
   ...
   */
  public static final String hashToHex(byte[] input, int len, long seed) {
     return toHexString(hash64(input, len, seed));
  }
   ...
   * @return the parsed long value, or -1 if invalid
   */
//  public static final long parseHexDigest(String hex) {
...
//  }
  ...
  private static final long xxh3_len_0to16(byte[] input, int len) {
     if (len > 8) {
        ...
        long input1 = integerToUnsignedLong(readLE32(input, 0));
        long input2 = integerToUnsignedLong(readLE32(input, len - 4));
        ...
        long keyed = (integerToUnsignedLong(combined)) ^ (XXH3_SECRET[0] ^ XXH3_SECRET[1]);
        return xxh64_avalanche(keyed);
     }
     return xxh64_avalanche(XXH3_SECRET[0] ^ XXH3_SECRET[1]);
  }

  private static final long xxh3_len_17to128(byte[] input, int len) {
  ...
  private static final long xxh3_len_129to240(byte[] input, int len) {
     long acc = len * XXH_PRIME64_1;
     int nbRounds = len / 16;
     for (int i = 0; i < 8; i++) {
  ...
  private static final long xxh3_hashLong(byte[] input, int len) {
  ...
  private static final long xxh3_accumulate_lane(long acc, byte[] input, int offset, int lane) {
  ...
  private static final long xxh3_mix_acc(long acc, int lane) {
     return (acc ^ (acc >>> 47) ^ XXH3_SECRET[lane % 8]) * XXH_PRIME64_1;
  }

  private static final long xxh3_mix16B(byte[] input, int offset, int secretIndex) {
  ...
  private static final long xxh3_mul128_fold64(long lo, long hi) {
  ...
  private static final long xxh3_avalanche(long h64) {
     h64 ^= h64 >>> 37;
  ...
  private static final long xxh3_rrmxmx(long h64, int len) {
  ...
  private static final long xxh64_avalanche(long h64) {
     h64 ^= h64 >>> 33;
     h64 *= XXH_PRIME64_2;
     h64 ^= h64 >>> 29;
     h64 *= XXH_PRIME64_3;
  ...
  private static final long readLE64(byte[] buf, int offset) {
  ...
  private static final int readLE32(byte[] buf, int offset) {
