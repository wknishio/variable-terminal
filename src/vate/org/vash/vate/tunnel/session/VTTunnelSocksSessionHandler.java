  ...
  public void run()
  {
    try
    {
      ...
      else
      {
        ...
        {
          //VTSocksProxyServer.setUDPTimeout(VT.VT_PING_LIMIT_MILLISECONDS);
          VTSocksProxyServer socksServer = new VTSocksProxyServer(new VTSocksHttpProxyAuthenticatorNone(channel.getConnection().getExecutorService(), bind, connectTimeout, dataTimeout, proxy), session.getSocket(), channel.getConnection().getExecutorService(), false, false, false, bind, connectTimeout, proxy);
          socksServer.setDatagramSocketFactory(channel.getConnection().createRemoteSocketFactory(channel));
          socksServer.setPipeBufferSize(VTSystem.VT_STANDARD_BUFFER_SIZE_BYTES);
          socksServer.setAcceptTimeout(connectTimeout);
          socksServer.setIdleTimeout(dataTimeout);
