...
//      }
      else if (parsed[1].equalsIgnoreCase("SL"))
      {
        ...
        {
          int sessionsLimit = server.getServerConnector().getSessionsLimit();
          VTConsole.print("\rVT>Session limit(SL): [" + sessionsLimit + "]\nVT>");
        }
        else if (parsed.length >= 3)
        {
          ...
          VTConsole.print("\rVT>Session limit(SL) set to: [" + sessionsLimit + "]\nVT>");
        }
        else
        {
          VTConsole.print("\rVT>Invalid command syntax!" + VTHelpManager.getHelpForServerCommand(parsed[0]));
      ...
      else if (parsed[1].equalsIgnoreCase("ET"))
      {
        ...
        else if (parsed.length >= 3)
        {
          ...
          //else if (encryptionType.toUpperCase().startsWith("B"))
          //{
            //VTConsole.print("\rVT>Encryption type(ET) set to: [BLOWFISH]\nVT>");
          //}
          else if (encryptionType.toUpperCase().startsWith("S"))
          {
