      ...
      if (splitCommand.length >= 2)
      {
        ...
        if (process_command == PROCESS_COMMAND_MANAGED || process_command == PROCESS_COMMAND_FREE)
        {
          try
          {
            ...
            processBuilder.environment().putAll(VTMainNativeUtils.getvirtualenv());
          ...
          if (process_scope == PROCESS_SCOPE_ALL)
          {
            ...
            for (VTRuntimeProcess process : managedProcessList.toArray(new VTRuntimeProcess[] {}))
            {
              try
              {
                ...
                try
                {
                  data = org.apache.commons.codecvt.binary.Base64.decodeBase64(commandData);
                }
                catch (Throwable e)
                {
                  synchronized (this)
          ...
          if (process_scope == PROCESS_SCOPE_ALL)
          {
            ...
            for (VTRuntimeProcess process : managedProcessList.toArray(new VTRuntimeProcess[] {}))
            {
              try
              {
                ...
                try
                {
                  data = org.apache.commons.lang3vt.StringEscapeUtils.unescapeJava(commandData).getBytes("UTF-8");
                }
                catch (Throwable e)
                {
                  synchronized (this)
