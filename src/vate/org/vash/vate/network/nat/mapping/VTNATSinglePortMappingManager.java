  ...
  private void deleteUPNPPortMapping(InternetGatewayDevice[] devices, VTNATPortMapping mapping)
  {
    ...
    for (InternetGatewayDevice device : devices)
    {
      ...
      try
      {
        ...
        Thread.sleep(250);
        /*
         * ActionResponse response = device.getSpecificPortMappingEntry(remoteHost,
         * externalPort, protocol); if (response != null) { String current =
         * response.getOutActionArgumentValue( "NewPortMappingDescription"); if
         * (description.equals(current)) { Thread.sleep(500);
         * device.deletePortMapping(remoteHost, externalPort, protocol); } }
         */
      }
      catch (Throwable e)
  ...
  private void setUPNPPortMapping(InternetGatewayDevice[] devices, VTNATPortMapping mapping)
  {
    ...
    for (InternetGatewayDevice device : devices)
    {
      ...
      try
      {
        ...
        Thread.sleep(250);
        // boolean ok = device.addPortMapping(description, remoteHost, internalPort,
        // externalPort, localAddress, 0, protocol);
        // System.out.println("setUPNPPortMapping.ok=" + ok);
        /*
         * ActionResponse response = device.getSpecificPortMappingEntry(remoteHost,
         * externalPort, protocol); if (response == null) { Thread.sleep(500);
         * device.addPortMapping(description, remoteHost, internalPort, externalPort,
         * localAddress, leaseTime, protocol); } else { String current =
         * response.getOutActionArgumentValue( "NewPortMappingDescription"); if
         * (description.equals(current)) { Thread.sleep(500);
         * device.addPortMapping(description, remoteHost, internalPort, externalPort,
         * localAddress, leaseTime, protocol); } }
         */
      }
      catch (Throwable e)
      {
