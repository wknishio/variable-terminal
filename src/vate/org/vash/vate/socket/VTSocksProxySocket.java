...
public class VTSocksProxySocket extends VTProxySocket
{
  ...
  public void connect(SocketAddress endpoint, int timeout) throws IOException
  {
    if (proxySocket == null)
    {
      ...
      {
        InetSocketAddress host = (InetSocketAddress) endpoint;
        SocksSocket socksSocket = new SocksSocket(proxyClient5, host.getHostName(), host.getPort(), timeout);
        proxySocket = socksSocket;
      }
      catch (Throwable t)
      {
      ...
      {
        InetSocketAddress host = (InetSocketAddress) endpoint;
        SocksSocket socksSocket = new SocksSocket(proxyClient4, host.getHostName(), host.getPort(), timeout);
        proxySocket = socksSocket;
      }
      catch (Throwable t)
      {
