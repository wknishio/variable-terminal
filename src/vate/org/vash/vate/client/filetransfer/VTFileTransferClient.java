...
public class VTFileTransferClient extends VTTask
{
  ...
  /*
   * public String[] split(String[] splitCommand, String command) { int x, y, z; y
   * = 0; z = 0; for (x = 0; x < command.length(); x++) { if (command.charAt(x) ==
   * '|') { y++; } } splitCommand = new String[++y]; y = 0; for (x = 0; x <=
   * command.length(); x++) { if ((x == command.length()) || (command.charAt(x) ==
   * '|')) { splitCommand[y++] = command.substring(z, x); z = ++x; } } return
   * splitCommand; }
   */
