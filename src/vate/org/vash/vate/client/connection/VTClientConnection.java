...
//	}
  ...
  public boolean setConnectionStreams(byte[] digestedCredentials) throws IOException
  {
    ...
    cryptoEngine.initializeClientEngine(encryptionType, encryptionKey, localNonce, remoteNonce, digestedCredentials);
...
//  }
  ...
  private int discoverRemoteEncryptionType(byte[] localNonce, byte[] remoteNonce, byte[] encryptionKey, byte[] localCheckString, int encryptionType) throws IOException
  {
    ...
    if (VTArrayComparator.arrayEquals(digestedServer, blake3Digest.digest(VT.VT_SECURITY_DIGEST_SIZE_BYTES, VT_SERVER_CHECK_STRING_LEA)))
    {
...
//      }
    ...
    {
      remoteEncryptionType = discoverRemoteEncryptionType(localNonce, remoteNonce, encryptionKey, VT_CLIENT_CHECK_STRING_LEA, encryptionType);
      if (remoteEncryptionType != -1)
      {
        setEncryptionType(VT.VT_CONNECTION_ENCRYPT_LEA);
        return true;
