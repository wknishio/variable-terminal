...
public class VTCompressorSelector
{
  ...
  public static OutputStream createFlushBufferedSyncFlushDeflaterOutputStreamFilteredStrategy(OutputStream out)
  {
    ...
    try
    {
      ...
      javaDeflater.setLevel(Deflater.BEST_SPEED);
      VTSyncFlushDeflaterOutputStream javaDeflaterOutputStream = new VTSyncFlushDeflaterOutputStream(out, javaDeflater, VT.VT_COMPRESSED_DATA_BUFFER_SIZE);
      return new VTBufferedOutputStream(javaDeflaterOutputStream, VT.VT_COMPRESSED_DATA_BUFFER_SIZE, false);
    }
    catch (Throwable t)
    {
      try
      {
        ...
        jzlibDeflater.setFlushMode(JZlib.Z_SYNC_FLUSH);
        jzlibDeflater.getZStream().deflateParams(JZlib.Z_BEST_SPEED, JZlib.Z_FILTERED);
        return new VTBufferedOutputStream(jzlibDeflater, VT.VT_COMPRESSED_DATA_BUFFER_SIZE, false);
      }
      catch (Throwable e)
      {
  ...
  {
    // return out;
    return new LZ4BlockOutputStream(out, VT.VT_COMPRESSED_DATA_BUFFER_SIZE, LZ4Factory.safeInstance().fastCompressor(), XXHashFactory.disabledInstance().newStreamingHash32(0x9747b28c).asChecksum(), true);
  ...
  public static OutputStream createBufferedLz4OutputStream(OutputStream out)
  {
    ...
    return new VTBufferedOutputStream(new LZ4BlockOutputStream(out, VT.VT_COMPRESSED_DATA_BUFFER_SIZE, LZ4Factory.safeInstance().fastCompressor(), XXHashFactory.disabledInstance().newStreamingHash32(0x9747b28c).asChecksum(), true), VT.VT_COMPRESSED_DATA_BUFFER_SIZE, false);
  ...
  public static InputStream createBufferedLz4InputStream(InputStream in)
  {
    // return in;
    // return createFlushBufferedSyncFlushInflaterInputStream(in);
    return new BufferedInputStream(new LZ4BlockInputStream(in, LZ4Factory.safeInstance().fastDecompressor(), XXHashFactory.disabledInstance().newStreamingHash32(0x9747b28c).asChecksum(), false), VT.VT_COMPRESSED_DATA_BUFFER_SIZE);
  ...
  public static OutputStream createFlushBufferedSnappyOutputStream(OutputStream out)
  {
    ...
    try
    {
      return new VTBufferedOutputStream(new SnappyOutputStream(out, false), VT.VT_COMPRESSED_DATA_BUFFER_SIZE, false);
    }
    catch (Throwable t)
    {
