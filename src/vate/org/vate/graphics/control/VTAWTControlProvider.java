package org.vate.graphics.control;
...
public final class VTAWTControlProvider
{
	...
	public final boolean initializeInputControl()
	{
		...
		/* dispose(); try { if (interfaceInputRobot == null) {
		 * interfaceInputRobot = new Robot();
		 * interfaceInputRobot.setAutoDelay(0);
		 * interfaceInputRobot.setAutoWaitForIdle(false); }
		 * inputControlInitialized = true; return true; } catch (Throwable e) {
		 * inputControlInitialized = false; return false; } */
	}
	...
	private final boolean initializeInputControl(GraphicsDevice device)
	{
		...
		try
		{
			if (interfaceInputRobot == null)
			{
				 ...
				 * getDeviceBounds(device); if (deviceBounds.x == 0 &&
				 * deviceBounds.y == 0) { interfaceInputRobot = new
				 * Robot(device); } else { interfaceInputRobot = new Robot(); }
				 * } else { interfaceInputRobot = new Robot(); } */
				interfaceInputRobot = new Robot();
				// interfaceInputRobot = new Robot();
