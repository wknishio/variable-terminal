...
public final class VTQuadrupleOctalTreeFrameDifferenceCodecV7
{
  // iterators
  // macroblock count
  // private int c0 = 0;
  ...
  public VTQuadrupleOctalTreeFrameDifferenceCodecV7()
  {
  ...
  private static final void encodePixelDirect(final VTLittleEndianOutputStream out, final byte[] oldPixelData, final byte[] newPixelData, final int position, final int x, final int y, final int width) throws IOException
  {
    ...
    // int pred = Math.max(Math.min(left, top), Math.min(Math.max(left, top), left +
    // top - diag));
