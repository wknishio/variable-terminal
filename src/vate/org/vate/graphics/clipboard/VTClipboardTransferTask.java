package org.vate.graphics.clipboard;
...
import org.vate.graphics.image.VTTransferableImage;
			...
			if (in.read() == VT.VT_GRAPHICS_MODE_CLIPBOARD_TRANSFER_STARTABLE)
			{
				if (sending)
				{
					try
					{
						...
						else if (transferable.isDataFlavorSupported(DataFlavor.imageFlavor))
						{
							...
							VTTransferableImage image = new VTTransferableImage(vtImageIO, (Image) transferable.getTransferData(DataFlavor.imageFlavor), recyclableDataBuffer);
				...
				else
				{
					try
					{
						...
						{
							VTTransferableImage image = new VTTransferableImage(vtImageIO, recyclableDataBuffer);
							recyclableDataBuffer = image.getRecyclableDataBuffer();
							image.read(in);
