...
public final class VTPipedInputStream extends InputStream
{
	...
	/* private void putFlowControl() throws IOException { while(freeSpace() ==
	 * 0) { if(eof) { throw new IOException("InputStreamPipe already got eof");
	 * } if(closed) { throw new IOException("InputStreamPipe closed"); }
	 ...
	 * java.io.PipedOutputStream(in);
	 * final byte[] msg = new byte[4711]; for(int i = 0; i < 4711; i++) { msg[i]
	 * = (byte)((i i) ^ (i + i)); } Thread w = new Thread(new Runnable() {
	 * public void run() { try { for(int i = 0; i < 1000; i++) { out.write(msg);
	 * } } catch (IOException e) { System.out.println("Error in w: " + e);
	 * e.printStackTrace(); } } }); Thread r = new Thread(new Runnable() {
	 * public void run() { try { byte[] imsg = new byte[4711]; for(int i = 0; i
	 * < 1000; i++) { int l = 4711; int o = 0; while(o < 4711) { int n =
	 * in.read(imsg, o, l); o += n; l -= n; } } } catch (IOException e) {
	 * System.out.println("Error in w: " + e); e.printStackTrace(); } } }); long
	 * start = System.currentTimeMillis(); System.out.println("Start: " + (start
	 * / 1000)); w.start(); r.start(); r.join(); long now =
	 * System.currentTimeMillis(); System.out.println("End: " + (now / 1000));
