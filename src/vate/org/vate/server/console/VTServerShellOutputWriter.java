...
public class VTServerShellOutputWriter extends VTTask
{
	...
	public void run()
	{
		while (!stopped)
		{
			try
			{
				/* if (session.getShellErrorReader().ready()) { readChars =
				 * session.getShellErrorReader().read(resultBuffer, 0,
				 * resultBufferSize); if (readChars > 0 && !stopped) {
				 * connection.getResultWriter().write(resultBuffer, 0,
				 * readChars); connection.getResultWriter().flush(); } else {
				 * stopped = true; return; } } else */
				/* if (session.getShellOutputReader().ready()) { readChars =
				 * session.getShellOutputReader().read(resultBuffer, 0,
				 * resultBufferSize); if (readChars > 0 && !stopped) {
				 * connection.getResultWriter().write(resultBuffer, 0,
				 * readChars); connection.getResultWriter().flush(); } else {
				 * stopped = true; break; } } else { Thread.sleep(1); } */
