...
public class VTGraphicsModeClientRemoteInterface extends Canvas
{
	...
	public void refreshImage()
	{
		...
		&& displayImageGraphics != null
		&& displayImageDataBuffer != null
		&& sourceImageDataBuffer.getType() != BufferedImage.TYPE_INT_RGB
		&& sourceImageDataBuffer.getType() != BufferedImage.TYPE_INT_ARGB
		&& sourceImageDataBuffer.getType() != BufferedImage.TYPE_USHORT_555_RGB)
		{
	...
	public void setImage(BufferedImage image)
	{
		if (image != null)
		{
			...
			if (sourceImageDataBuffer == null
			|| sourceImageDataBuffer.getType() != image.getType()
			|| sourceImageDataBuffer.getWidth() != image.getWidth()
			|| sourceImageDataBuffer.getHeight() != image.getHeight()
			|| sourceImageDataBuffer != image
			|| !nextSize.equals(getSize()))
			{
				try
				{
					sourceImageDataBuffer = image;
	...
	/* public synchronized Dimension getImageSize() { if (image != null) {
	 * return new Dimension(image.getWidth(), image.getHeight()); } return
	 * this.getSize(); } */
	...
	public void paint(Graphics g)
	{
		...
		if (sourceImageDataBuffer != null)
		{
			...
			try
			{
				if (sourceImageDataBuffer.getType() != BufferedImage.TYPE_INT_RGB
				&& sourceImageDataBuffer.getType() != BufferedImage.TYPE_INT_ARGB
				&& sourceImageDataBuffer.getType() != BufferedImage.TYPE_USHORT_555_RGB)
				{
