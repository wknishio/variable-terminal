...
public class VTClientLegacyRemoteConsoleWriter extends VTTask
{
	private StringBuilder message;
	private VTClientSession session;
	private VTClientConnection connection;
	
	public VTClientLegacyRemoteConsoleWriter(VTClientSession session)
	{
	...
	private void executeCommand(String command, Set<String> stack) throws Throwable
	{
		...
		else if (command != null)
		{
			...
			else if (parsed[0].equalsIgnoreCase("*VTQUIT") || parsed[0].equalsIgnoreCase("*VTQT"))
			{
				connection.getCommandWriter().write(command + "\n");
				connection.getCommandWriter().flush();
			...
			else if (parsed[0].equalsIgnoreCase("*VTSAVE") || parsed[0].equalsIgnoreCase("*VTSV"))
			{
				if (parsed.length == 1)
				{
					try
