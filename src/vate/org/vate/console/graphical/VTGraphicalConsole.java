...
public class VTGraphicalConsole implements VTConsoleImplementation
{
	...
	private VTGraphicalConsole()
	{
		...
		textArea.setCaretPosition(0);
		//VTGraphicalConsoleTextAreaScrollPane scrolling = new VTGraphicalConsoleTextAreaScrollPane(textArea);
	 ...
	 * overwriteLineCount = caretRecoilLines; }
	 * public static int getCaretRecoilLines() { return overwriteLineCount; } */
	...
	public static void increaseCharacterCount()
	{
		if (overwriteCharactersCount == 0)
		{
			/* if (outputBuffer.length() >= maxCharacterCount) {
			 * deleteLineInBuffer(); } */
	...
	/* private static void buildReplacement(StringBuilder str, int start, int
	 * end, StringBuilder out, String replacement) {
	 * //System.out.println(start); //System.out.println(end);
	 * //System.out.println(str.substring(start, end)); String[] values =
	 * str.substring(start, end).split("\n"); System.out.println("substring: ["
	 * + str.substring(start, end) + "]"); System.out.println("replacement: [" +
	 * replacement + "]"); int appended = 0; int remaining =
	 * replacement.length(); int min = 0; System.out.println("values: " +
	 * values.length); for (String value : values) { out.append("\n"); if
	 * (remaining > 0) { System.out.println("appended!");
	...
	/* private static int countOccurencesInString(char c, String str) { int
	 * count = 0; for (int i = 0;i < str.length();i++) { if (str.charAt(i) == c)
	 * { count++; } } return count; } */
	 ...
	 * } } updatingTerminal = true; int targetLength = str.length(); int
	 * currentLength = targetLength; if (targetLength > (currentLength -
	 ...
	 * totalCharactersCount)))) { currentLength++; } }
	 ...
	 * str); System.out.println("spacesBuffer.length: " +
	 * spacesBuffer.length()); System.out.println("spacesBuffer: [" +
	 * spacesBuffer.toString() + "]"); screenBuffer.replace(totalCharactersCount
	 * - spacesBuffer.length(), totalCharactersCount, spacesBuffer.toString());
	 * String spacesBufferContents = spacesBuffer.toString().replace('\r', '
	 ...
	 * } } finally { synchronized (outputSynchronizer) { updatingTerminal =
	 * false; outputSynchronizer.notify(); } } } */
	...
	public static void output(char c)
	{
		try
		{
			synchronized (outputSynchronizer)
			{
				...
				{
					/* outputBuffer.setLength(0);
					 * textArea.replaceRange(replacedBlankArea, 0,
					 * maxCharactersCount); screenBuffer.replace(0,
					 * maxCharactersCount, trueBlankArea);
					 * //textArea.setCaretPosition(0); totalCharactersCount = 0;
					 * lineCount = 0; charactersInLineCount = 0;
					 * overwriteCharactersCount = 0;
	...
	/* private static int deleteTabInBufferBackspacing() { int deleted = 1;
	 * while (outputBuffer.length() > 0 &&
	 * outputBuffer.charAt(outputBuffer.length()) == '\t' && deleted < tabSize) {
	 * reduceCharacterCount(); deleted++; outputBuffer.insert(0, '\b'); } return
	 * deleted - 1; } */
		...
		try
		{
			synchronized (outputSynchronizer)
			{
				...
				if (totalCharactersCount - overwriteCharactersCount > 0)
				{
					...
					{
						/* if (flushInterrupted) { if (outputBuffer.length() >
						 * 0) { if (outputBuffer.charAt(outputBuffer.length() -
						 * 1 - overwriteCharactersCount) == '\t') {
						 * outputBuffer.deleteCharAt(outputBuffer.length() - 1 -
						 * overwriteCharactersCount); deleteTabInBuffer(); }
						 * else {
						 * outputBuffer.deleteCharAt(outputBuffer.length() - 1 -
	...
	public static char read() throws InterruptedException
	{
		synchronized (inputSynchronizer)
		{
			...
			{
				/* try { inputSynchronizer.wait(); } catch (InterruptedException
				 * e) { //VTTerminal.println(e.getMessage()); } */
	...
	public String getSelectedText()
	{
		try
		{
			synchronized (outputSynchronizer)
			{
				...
				if (/* textArea.getCaretPosition() == totalCharactersCount -
					 * overwriteCharactersCount //&&
					 * textArea.getTrueCaretPosition() == totalCharactersCount -
					 * overwriteCharactersCount && selectionStart ==
					 * totalCharactersCount - overwriteCharactersCount &&
					 * selectionEnd == totalCharactersCount -
					 * overwriteCharactersCount + 1 || */ selectionStart == selectionEnd)
				{
					return null;
				}
				return screenBuffer.substring(selectionStart, selectionEnd).replace("\b", "").replace("\n", "").replaceAll("\\t{1," + tabSize + "}", "\t").replace('\r', '\n');
