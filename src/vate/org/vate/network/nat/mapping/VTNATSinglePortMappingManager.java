	...
	private void deleteUPNPPortMapping(InternetGatewayDevice[] devices, VTNATPortMapping mapping)
	{
		...
		for (InternetGatewayDevice device : devices)
		{
			...
			try
			{
				 ...
				 * response.getOutActionArgumentValue(
				 * "NewPortMappingDescription"); if
	...
	private void setUPNPPortMapping(InternetGatewayDevice[] devices, VTNATPortMapping mapping)
	{
		...
		for (InternetGatewayDevice device : devices)
		{
			...
			try
			{
				 ...
				 * device.addPortMapping(description, remoteHost, internalPort,
				 * externalPort, localAddress, leaseTime, protocol); } else {
				 * String current = response.getOutActionArgumentValue(
				 * "NewPortMappingDescription"); if
				 * (description.equals(current)) { Thread.sleep(500);
				 * device.addPortMapping(description, remoteHost, internalPort,
				 * externalPort, localAddress, leaseTime, protocol); } } */
			}
			catch (Throwable e)
			{
				//e.printStackTrace();
