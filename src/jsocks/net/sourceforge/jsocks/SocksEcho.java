...
public class SocksEcho extends Frame implements ActionListener, Runnable, WindowListener {
	...
	Button proxy_button, accept_button, clear_button, connect_button, udp_button, quit_button;
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (source == proxy_button)
			...
			onQuit();
		else if (source == connect_button || source == port_text || source == host_text)
			onConnect();
	...
	private void doUDPPipe() throws IOException {
		DatagramPacket dp = new DatagramPacket(new byte[MAX_DATAGRAM_SIZE], MAX_DATAGRAM_SIZE);
		while (true) {
			udp_sock.receive(dp);
