...
public class SocksDialog extends Dialog implements WindowListener, ItemListener, ActionListener, Runnable {
	 ...
	 * <p>
	 * Default value is true, however on some JVMs, namely one from the Microsoft,
	 * it doesn't want to work properly, separate thread can't close the dialog
	 * opened in GUI thread, and everuthing else is then crashes.
	 * <p>
	 * When setting this variable to false, SocksDialog will block while trying to
	 * look up proxy host, and if this takes considerable amount of time it might be
	 * annoying to user.
	 * 
	 */
	 ...
	 * <p>
	 * Returns initialised proxy object, or null if user cancels dialog by either
	 * pressing Cancel or closing the dialog window.
	 */
	 ...
	 * args) throws Exception{ Frame f = new Frame("Test for SocksDialog");
	 * f.add("Center", new Label("Fill the Dialog")); SocksDialog socksdialog = new
	 * SocksDialog(f); f.pack(); f.show(); f.addWindowListener(socksdialog); Proxy p
