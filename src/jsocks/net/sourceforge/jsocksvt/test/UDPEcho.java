...
public class UDPEcho implements Runnable {
	...
	public void run() {
		...
		try {
			while (true) {
				...
				System.out.println(
						"UDP From:" + incomingData.getAddress().getHostAddress() + ":" + incomingData.getPort());
	...
	public static void doEcho(int port) throws IOException {
		...
		System.out
				.println("Starting UDP echo on" + sock.getLocalAddress().getHostAddress() + ":" + sock.getLocalPort());
		while (true) {
			try {
				sock.receive(packet);
				sock.send(packet);
