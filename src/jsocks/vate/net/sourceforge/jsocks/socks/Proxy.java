...
public abstract class Proxy {
	 ...
	 * discarded, any changes done previously with addDirect(Inetaddress) will be
	 * lost. The machines in this range will be accessed without using proxy.
	 * 
	 * @param ir
	 ...
	 * 
	 * @return Proxy which is used to connect to this proxy, or null if proxy is to
	...
	/**
	 * Parses strings in the form: host[:port:user:password], and creates proxy from
	 ...
	 * @return Proxy created from the string, null if entry was somehow invalid(host
	 *         unknown for example, or empty string)
	 */
	...
	public static Proxy buildProxy(String proxy_host, String s_proxy_port, String proxy_user, String proxy_password) {

		Proxy proxy;
		int proxy_port = 1080;
		if (s_proxy_port != null) {
			try {
				proxy_port = Integer.parseInt(s_proxy_port);
			} catch (Exception e) {
	...
	/**
	 * Sends the request reads reply and returns it throws exception if something
	 * wrong with IO or the reply code is not zero
	 */
