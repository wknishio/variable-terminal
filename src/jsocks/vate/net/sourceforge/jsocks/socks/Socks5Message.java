...
public class Socks5Message extends ProxyMessage {
	...
	/**
	 * Initializes Message from the stream. Reads server response or client request
	 * from given stream.
	 * 
	 * @param in
	 *            Input stream to read response from.
	...
	/**
	 * Returns IP field of the message as IP, if the message was created with ATYP
	 * of HOSTNAME, it will attempt to resolve the hostname, which might fail.
	 * 
	...
	/**
	 * Wether to resolve hostIP returned from SOCKS server that is wether to create
	 * InetAddress object from the hostName string
	 */
