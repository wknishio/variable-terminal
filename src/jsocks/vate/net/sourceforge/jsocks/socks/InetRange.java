...
public class InetRange implements Cloneable {
	 ...
	 * In which case anything that ends with .myhost.net.au will be considered in
	 * the range.
	 * 
	 * <li>Range in the form ddd.ddd.ddd. <BR>
	 * This will be treated as range ddd.ddd.ddd.0 to ddd.ddd.ddd.255. It is not
	 * necessary to specify 3 first bytes you can use just one or two. For example
	 * 130. will cover address between 130.0.0.0 and 13.255.255.255.
	 * 
	 ...
	 * than or equal to the address of from and smaller than or equal to the address
	 * of to will be included in the range.
	 * 
	 * @param from
	...
	/**
	 * Checks wether the givan host is in the range. Attempts to resolve host name
	 * if required.
	 * 
	 * @param host
	 *            Host name to check.
	 ...
	 * range.
	 * <li>If attemptResolve is true, host is name, rather than ip, and all previous
	 * attempts failed, try to resolve the hostname, and check wether the ip
	 * associated with the host is in the range.It also repeats all previos steps
	 * with the hostname obtained from InetAddress, but the name is not allways the
	 * full name,it is quite likely to be the same. Well it was on my machine.
	 * </ol>
	 * 
	 * @param host
	 *            Host name to check.
	...
	/////////////////
	/**
	 * Same as previous but used internally, to avoid unnecessary convertion of IPs,
	 * when checking subranges
	 */
	...
	private boolean checkHostEnding(String host) {
		...
		while (iterator.hasNext()) {
			if (host.endsWith((String) iterator.next()))
				return true;
		}
		return false;
	}
