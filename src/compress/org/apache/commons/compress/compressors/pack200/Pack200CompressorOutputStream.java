...
public class Pack200CompressorOutputStream extends CompressorOutputStream {
	...
	/**
	 * Compresses the given stream, caching the compressed data in memory and using
	...
	/**
	 * Compresses the given stream using the given strategy to cache the results and
	 * the given properties.
	 *
	 * @param out
	 *            the stream to write to
	...
	public void finish() throws IOException {
		if (!finished) {
			...
			try {
				p.pack(ji = new JarInputStream(streamBridge.getInput()), originalOutput);
				success = true;
			} finally {
				if (!success) {
