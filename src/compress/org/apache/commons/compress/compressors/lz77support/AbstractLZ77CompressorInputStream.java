 ...
 *
 * <p>
 * Assumes the stream consists of blocks of literal data and back-references
 * (called copies) in any order. Of course the first block must be a literal
 * block for the scheme to work - unless the {@link #prefill prefill} method has
 * been used to provide initial data that is never returned by {@link #read
 * read} but only used for back-references.
 * </p>
 *
 * <p>
 * Subclasses must override the three-arg {@link #read read} method as the
 * no-arg version delegates to it and the default implementation delegates to
 * the no-arg version, leading to infinite mutual recursion and a
 * {@code StackOverflowError} otherwise.
 * </p>
 *
 * <p>
 ...
 * <ul>
 *
 * <li>keep track of the current state of the stream. Is it inside a literal
 * block or a back-reference or in-between blocks?</li>
 *
 * <li>Use {@link #readOneByte} to access the underlying stream directly.</li>
 *
 * <li>If a new literal block starts, use {@link #startLiteral} to tell this
 * class about it and read the literal data using {@link #readLiteral} until it
 * returns {@code 0}. {@link #hasMoreDataInBlock} will return {@code false}
 * before the next call to {@link #readLiteral} would return {@code 0}.</li>
 *
 * <li>If a new back-reference starts, use {@link #startBackReference} to tell
 * this class about it and read the literal data using
 * {@link #readBackReference} until it returns {@code 0}.
 * {@link #hasMoreDataInBlock} will return {@code false} before the next call to
 * {@link #readBackReference} would return {@code 0}.</li>
 *
 * <li>If the end of the stream has been reached, return {@code -1} as this
 * class' methods will never do so themselves.</li>
 *
 * </ul>
 *
 * <p>
 * {@link #readOneByte} and {@link #readLiteral} update the counter for bytes
 * read.
 * </p>
 *
 * @since 1.14
...
public abstract class AbstractLZ77CompressorInputStream extends CompressorInputStream {
	...
	/**
	 * Buffer to write decompressed bytes to for back-references, will be three
	 * times windowSize big.
	 *
	 * <p>
	 * Three times so we can slide the whole buffer a windowSize to the left once
	 * we've read twice windowSize and still have enough data inside of it to
	 * satisfy back-references.
	 * </p>
	 */
	 ...
	 * @param windowSize
	 *            Size of the window kept for back-references, must be bigger than
	 *            the biggest offset expected.
	 *
	 * @throws IOException
	 *             if reading fails
	 */
	...
	/**
	 * Used by subclasses to signal the next block contains the given amount of
	 * literal data.
	 * 
	 * @param length
	 ...
	 * @param off
	 *            offset to start writing to
	 * @param len
	 *            maximum amount of data to read
	 * @return number of bytes read, may be 0. Will never return -1 as EOF-detection
	 *         is the responsibility of the subclass
	 * @throws IOException
	 *             if the underlying stream throws or signals an EOF before the
	 *             amount of data promised for the block have been read
	 */
	...
	private void tryToReadLiteral(int bytesToRead) throws IOException {
		// min of "what is still inside the literal", "what does the user want" and "how
		// muc can fit into the buffer"
		final int reallyTryToRead = Math.min((int) Math.min(bytesToRead, bytesRemaining), buf.length - writeIndex);
		final int bytesRead = reallyTryToRead > 0 ? IOUtils.readFully(in, buf, writeIndex, reallyTryToRead)
	...
	/**
	 * Used by subclasses to signal the next block contains a back-reference with
	 * the given coordinates.
	 * 
	 * @param offset
	 *            the offset of the back-reference
	 * @param length
	 *            the length of the back-reference
	 */
	...
	private void tryToCopy(int bytesToCopy) {
		...
		int copy = Math.min((int) Math.min(bytesToCopy, bytesRemaining), buf.length - writeIndex);
		if (copy == 0) {
			// NOP
