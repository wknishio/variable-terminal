 ...
 *
 * <p>
 * Most LZ77 derived algorithms split input data into blocks of uncompressed
 * data (called literal blocks) and back-references (pairs of offsets and
 * lengths) that state "add <code>length</code> bytes that are the same as those
 * already written starting <code>offset</code> bytes before the current
 * position. The details of how those blocks and back-references are encoded are
 * quite different between the algorithms and some algorithms perform additional
 * steps (Huffman encoding in the case of DEFLATE for example).
 * </p>
 *
 * <p>
 * This class attempts to extract the core logic - finding back-references - so
 * it can be re-used. It follows the algorithm explained in section 4 of RFC
 * 1951 (DEFLATE) and currently doesn't implement the "lazy match" optimization.
 * The three-byte hash function used in this class is the same as the one used
 * by zlib and InfoZIP's ZIP implementation of DEFLATE. The whole class is
 * strongly inspired by InfoZIP's implementation.
 * </p>
 *
 * <p>
 * LZ77 is used vaguely here (as well as many other places that talk about it
 * :-), LZSS would likely be closer to the truth but LZ77 has become the synonym
 * for a whole family of algorithms.
 * </p>
 *
 * <p>
 * The API consists of a compressor that is fed <code>byte</code>s and emits
 * {@link Block}s to a registered callback where the blocks represent either
 * {@link LiteralBlock literal blocks}, {@link BackReference back-references} or
 * {@link EOD end of data markers}. In order to ensure the callback receives all
 * information, the {@code #finish} method must be used once all data has been
 ...
 * <dd>the size of the sliding window, must be a power of two - this determines
 * the maximum offset a back-reference can take. The compressor maintains a
 * buffer of twice of <code>windowSize</code> - real world values are in the
 * area of 32k.</dd>
 *
...
public class LZ77Compressor {
	 ...
	 *
	 * <p>
	 * For performance reasons this encapsulates the real data, not a copy of it.
	 * Don't modify the data and process it inside of {@link Callback#accept}
	...
	public static final class LiteralBlock extends Block {
		 ...
		 *
		 * <p>
		 * This returns a life view of the actual data in order to avoid copying, modify
		 * the array at your own risk.
		 * </p>
		 * 
		 * @return the data
		 */
	 ...
	 *
	 * <p>
	 * The callback is invoked on the same thread that receives the bytes to
	 * compress and may be invoked multiple times during the execution of
	...
	/**
	 * Feeds bytes into the compressor which in turn may emit zero or more blocks to
	 * the callback during the execution of this method.
	 * 
	 * @param data
	 *            the data to compress - must not be null
	 * @throws IOException
	...
	/**
	 * Tells the compressor to process all remaining data and signal end of data to
	 * the callback.
	 *
	 * <p>
	 * The compressor will in turn emit at least one block ({@link EOD}) but
	 * potentially multiple blocks to the callback during the execution of this
	 * method.
	 * </p>
	 * 
	 * @throws IOException
	 ...
	 *
	 * <p>
	 * This is used if the stream has been cut into blocks and back-references of
	 * one block may refer to data of the previous block(s). One such example is the
	 * LZ4 frame format using block dependency.
	 * </p>
	 *
	 * @param data
	 *            the data to fill the window with.
	...
	/**
	 * Assumes we are calculating the hash for three consecutive bytes as a rolling
	 * hash, i.e. for bytes ABCD if H is the hash of ABC the new hash for BCD is
	 * nextHash(H, D).
	 *
	 * <p>
	 * The hash is shifted by five bits on each update so all effects of A have been
	 * swapped after the third update.
	 * </p>
	 */
	...
	/**
	 * Inserts the current three byte sequence into the dictionary and returns the
	 * previous head of the hash-chain.
	 *
	 * <p>
	 * Updates <code>insertHash</code> and <code>prev</code> as a side effect.
	 * </p>
	 */
	...
	private int longestMatchForNextPosition(final int prevMatchLength) {
		// save a bunch of values to restore them if the next match isn't better than
		// the current one
	...
	private void insertStringsInMatch(int matchLength) {
		...
		// insertString inserts the byte 2 bytes after position, which may not yet be
		// available -> missedInserts
	...
	/**
	 * Searches the hash chain for real matches and returns the length of the
	 * longest match (0 if none were found) that isn't too far away (WRT maxOffset).
	 *
	 * <p>
	 * Sets matchStart to the index of the start position of the longest match as a
	 * side effect.
	 * </p>
	 */
