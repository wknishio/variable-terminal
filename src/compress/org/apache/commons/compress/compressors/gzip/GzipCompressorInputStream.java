...
/**
 * Input stream that decompresses .gz files. This supports decompressing
 * concatenated .gz files which is important when decompressing standalone .gz
 * files.
 * <p>
 * {@link java.util.zip.GZIPInputStream} doesn't decompress concatenated .gz
 * files: it stops after the first member and silently ignores the rest. It
 * doesn't leave the read position to point to the beginning of the next member,
 * which makes it difficult workaround the lack of concatenation support.
 * <p>
...
public class GzipCompressorInputStream extends CompressorInputStream {
	...
	/**
	 * Constructs a new input stream that decompresses gzip-compressed data from the
	 * specified input stream.
	 * <p>
	 * This is equivalent to
	 * <code>GzipCompressorInputStream(inputStream, false)</code> and thus will not
	 * decompress concatenated .gz files.
	 *
	 * @param inputStream
	 ...
	 * specified input stream.
	 * <p>
	 * If <code>decompressConcatenated</code> is {@code false}: This decompressor
	 * might read more input than it will actually use. If <code>inputStream</code>
	 * supports <code>mark</code> and <code>reset</code>, then the input position
	 * will be adjusted so that it is right after the last byte of the compressed
	 * stream. If <code>mark</code> isn't supported, the input position will be
	 * undefined.
	 *
	 * @param inputStream
	 *            the InputStream from which this object should be created of
	...
	/**
	 * Provides the stream's meta data - may change with each stream when
	 * decompressing concatenated streams.
	 * 
	...
	private boolean init(boolean isFirstMember) throws IOException {
		...
		if (magic0 != 31 || magic1 != 139) {
			throw new IOException(
					isFirstMember ? "Input is not in the .gz format" : "Garbage after a valid .gz stream");
		}
		...
		// Original file name
		if ((flg & FNAME) != 0) {
			parameters.setFilename(new String(readToNull(inData), CharsetNames.ISO_8859_1));
		}

		// Comment
		if ((flg & FCOMMENT) != 0) {
			parameters.setComment(new String(readToNull(inData), CharsetNames.ISO_8859_1));
		}
	...
	private long readLittleEndianInt(DataInputStream inData) throws IOException {
		return inData.readUnsignedByte() | (inData.readUnsignedByte() << 8) | (inData.readUnsignedByte() << 16)
