 ...
 *
 * @see <a href=
 *      "http://code.google.com/p/snappy/source/browse/trunk/framing_format.txt">Snappy
...
public class FramedSnappyCompressorInputStream extends CompressorInputStream {
	...
	private static final byte[] SZ_SIGNATURE = new byte[] { (byte) STREAM_IDENTIFIER_TYPE, // tag
			6, 0, 0, // length
			's', 'N', 'a', 'P', 'p', 'Y' };
	...
	public int available() throws IOException {
		if (inUncompressedChunk) {
			return Math.min(uncompressedBytesRemaining, in.available());
	 ...
	 *
	 * @return -1 if there is no current chunk or the number of bytes read from the
	 *         current chunk (which may be -1 if the end of the chunk is reached).
	 */
	...
	private void readNextBlock() throws IOException {
		...
		if (type == -1) {
			...
			throw new IOException(
					"unskippable chunk with type " + type + " (hex " + Integer.toHexString(type) + ")" + " detected.");
			...
			expectedChecksum = unmask(readCrc());
			currentCompressedChunk = new SnappyCompressorInputStream(new BoundedInputStream(in, size));
