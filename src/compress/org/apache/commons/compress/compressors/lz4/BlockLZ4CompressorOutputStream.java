...
public class BlockLZ4CompressorOutputStream extends CompressorOutputStream {
	...
	/*
	 * 
	 * The LZ4 block format has a few properties that make it less straight-forward
	 * than one would hope:
	 * 
	 * literal blocks and back-references must come in pairs (except for the very
	 * last literal block), so consecutive literal blocks created by the compressor
	 * must be merged into a single block.
	 * 
	 * the start of a literal/back-reference pair contains the length of the
	 * back-reference (at least some part of it) so we can't start writing the
	 * literal before we know how long the next back-reference is going to be.
	 * 
	 * there are special rules for the final blocks
	 * 
	 * > There are specific parsing rules to respect in order to remain > compatible
	 * with assumptions made by the decoder : > > 1. The last 5 bytes are always
	 * literals > > 2. The last match must start at least 12 bytes before end of >
	 * block. Consequently, a block with less than 13 bytes cannot be > compressed.
	 * 
	 * which means any back-reference may need to get rewritten as a literal block
	 * unless we know the next block is at least of length 5 and the sum of this
	...
	private LinkedList<byte[]> expandedBlocks = new LinkedList<byte[]>();
	...
	/**
	 * Compresses all remaining data and writes it to the stream, doesn't close the
	 * underlying stream.
	 * 
	 * @throws IOException
	 *             if an error occurs
	 ...
	 * @param off
	 *            offset of real data into the array
	 * @param len
	 *            amount of data
	 * @throws IllegalStateException
	 *             if the stream has already started to write data
	 * @see LZ77Compressor#prefill
	 */
	public void prefill(byte[] data, int off, int len) {
		if (len > 0) {
			byte[] b = copyOfRange(data, off, off + len);
	...
	private byte[] expand(final int offset, final int length) {
		...
		if (offset == 1) { // surprisingly common special case
			byte[] block = expandedBlocks.getFirst();
	...
	private void clearUnusedPairs() {
		int pairLengths = 0;
		int pairsToKeep = 0;
		for (Iterator<Pair> it = new DescendingIterator<Pair>(pairs.listIterator()); it.hasNext();) {
			Pair p = it.next();
			pairsToKeep++;
	...
	private void rewriteLastPairs() {
		...
		// * all other Pairs contain a back-reference which must be four bytes long at
		// minimum
	...
	public static Parameters.Builder createParameterBuilder() {
		...
		return Parameters.builder(BlockLZ4CompressorInputStream.WINDOW_SIZE)
				.withMinBackReferenceLength(MIN_BACK_REFERENCE_LENGTH).withMaxBackReferenceLength(maxLen)
				.withMaxOffset(maxLen).withMaxLiteralLength(maxLen);
	}

	final static class Pair {
		private final LinkedList<byte[]> literals = new LinkedList<byte[]>();
		...
		byte[] addLiteral(LZ77Compressor.LiteralBlock block) {
			byte[] copy = copyOfRange(block.getData(), block.getOffset(), block.getOffset() + block.getLength());
			literals.add(copy);
			return copy;
		}
		...
		private void prependTo(Pair other) {
			ListIterator<byte[]> listBackwards = literals.listIterator();
			while (listBackwards.hasPrevious()) {
				other.prependLiteral(listBackwards.previous());
			}
		}
	...
	private class DescendingIterator<E> implements Iterator<E> {
		...
		private DescendingIterator(ListIterator<E> iterator) {
