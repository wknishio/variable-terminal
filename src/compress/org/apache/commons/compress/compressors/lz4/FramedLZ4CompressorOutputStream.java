 ...
 * </p>
 *
 * @see <a href="http://lz4.github.io/lz4/lz4_Frame_format.html">LZ4 Frame
...
public class FramedLZ4CompressorOutputStream extends CompressorOutputStream {
	...
	public static class Parameters {
		...
		/**
		 * The default parameters of 4M block size, enabled content checksum, disabled
		 * block checksums and independent blocks.
		 *
		 * <p>
		...
		/**
		 * Sets up custom a custom block size for the LZ4 stream but otherwise uses the
		 * defaults of enabled content checksum, disabled block checksums and
		 * independent blocks.
		 * 
		 * @param blockSize
		 ...
		 *            the size of a single block.
		 * @param lz77params
		 *            parameters used to fine-tune compression, in particular to balance
		 ...
		 * @param blockSize
		 *            the size of a single block.
		 * @param withContentChecksum
		 *            whether to write a content checksum
		 * @param withBlockChecksum
		 *            whether to write a block checksum. Note that block checksums are
		 *            not supported by the lz4 command line utility
		 * @param withBlockDependency
		 *            whether a block may depend on the content of a previous block.
		 *            Enabling this may improve compression ratio but makes it
	 ...
	 * 
	 * @param out
	 *            the OutputStream to which to write the compressed data
	 * @throws IOException
	...
	/**
	 * Constructs a new output stream that compresses data using the LZ4 frame
	 * format using the given block size.
	 * 
	 * @param out
	 *            the OutputStream to which to write the compressed data
	 * @param params
	...
	private void flushBlock() throws IOException {
		...
		BlockLZ4CompressorOutputStream o = null;
		try {
			o = new BlockLZ4CompressorOutputStream(baos, params.lz77params);
