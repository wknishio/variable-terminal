...
public class FramedLZ4CompressorInputStream extends CompressorInputStream {
	...
	static final byte[] LZ4_SIGNATURE = new byte[] { // NOSONAR
			4, 0x22, 0x4d, 0x18 };
	private static final byte[] SKIPPABLE_FRAME_TRAILER = new byte[] { 0x2a, 0x4d, 0x18 };
	...
	/**
	 * Creates a new input stream that decompresses streams compressed using the LZ4
	 * frame format and stops after decompressing the first frame.
	 * 
	 * @param in
	 *            the InputStream from which to read the compressed data
	 * @throws IOException
	 *             if reading fails
	 */
	 ...
	 *            after the first LZ4 frame and leave the input position to point to
	 *            the next byte after the frame stream
	 * @throws IOException
	 *             if reading fails
	 */
	...
	/**
	 * Skips over the contents of a skippable frame as well as skippable frames
	 * following it.
	 *
	 * <p>
	 * It then tries to read four more bytes which are supposed to hold an LZ4
	 * signature and returns the number of bytes read while storing the bytes in the
	 * given array.
	 * </p>
	 */
