 ...
 * {@link BZip2CompressorInputStream} use
 * </p>
 *
 * <pre>
 ...
 *
 * <p>
 * For decompression {@code BZip2CompressorInputStream} allocates less memory if
 * the bzipped input is smaller than one block.
 * </p>
 *
 * <p>
...
public class BZip2CompressorOutputStream extends CompressorOutputStream implements BZip2Constants {
	 ...
	 *         {@code inputLength} this method returns {@code MAX_BLOCKSIZE} always.
	 *
	 * @param inputLength
	...
	/**
	 * Constructs a new {@code BZip2CompressorOutputStream} with a blocksize of
	 * 900k.
	 *
	 * @param out
	...
	/**
	 * Constructs a new {@code BZip2CompressorOutputStream} with specified
	 * blocksize.
	 *
	 * @param out
	 *            the destination stream.
	...
	/**
	 * Writes the current byte to the buffer, run-length encoding it if it has been
	 * repeated at least four times (the first step RLEs sequences of four identical
	 * bytes).
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * "write to the buffer" means adding to data.buffer starting two steps "after"
	 * this.last - initially starting at index 1 (not 0) - and updating this.last to
	 * point to the last index written minus 1.
	 * </p>
	 */
	...
	/**
	 * Writes magic bytes like BZ on the first position of the stream and bytes
	 * indiciating the file-format, which is huffmanised, followed by a digit
	...
	private void endBlock() throws IOException {
		 ...
		 * compression/ decompression donot rely on these statistical properties. They
		 * are only important when trying to recover blocks from damaged files.
		 */
		bsPutUByte(0x31);
		bsPutUByte(0x41);
		bsPutUByte(0x59);
	...
	/**
	 * Keeps track of the last bytes written and implicitly performs run-length
	...
	/*
	 * Performs Move-To-Front on the Burrows-Wheeler transformed buffer, storing the
	 * MTFed data in data.sfmap in RUNA/RUNB run-length-encoded form.
	 *
	...
	static final class Data {
		...
		/*
		 * holds the RLEd block of original data starting at index 1. After sorting the
		 * last byte added to the buffer is at index 0.
		 */
		final byte[] block; // 900021 byte
		/*
		 * maps index in Burrows-Wheeler transformed block => index of byte in original
		 * block
		 */
