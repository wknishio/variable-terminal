...
public class ArArchiveInputStream extends ArchiveInputStream {
	...
	public ArArchiveEntry getNextArEntry() throws IOException {
			...
			for (int i = 0; i < expected.length; i++) {
				if (expected[i] != realized[i]) {
					throw new IOException(
							"invalid entry trailer. not read the content? Occured at byte: " + getBytesRead());
				}
			}
		}
		...
		// GNU ar uses a '/' to mark the end of the filename; this allows for the use of
		// spaces without the use of an extended filename.
		...
		currentEntry = new ArArchiveEntry(temp, len, userId, asInt(ID_BUF, true), asInt(FILE_MODE_BUF, 8),
	...
	/**
	 * Does the name look like it is a long name (or a name containing spaces) as
	 * encoded by BSD ar?
	 *
	 * <p>
	...
	/**
	 * Reads the real name from the current stream assuming the very first bytes to
	 * be read are the real file name.
	 *
	 * @see #isBSDLongName
	 *
	 * @since 1.3
	...
	/**
	 * Is this the name of the "Archive String Table" as used by SVR4/GNU to store
	 * long file names?
	 *
	 * <p>
	 * GNU ar stores multiple extended filenames in the data section of a file with
	 * the name "//", this record is referred to by future headers.
	 * </p>
	 *
	 * <p>
	 * A header references an extended filename by storing a "/" followed by a
	 * decimal offset to the start of the filename in the extended filename data
	 * section.
	 * </p>
	 * 
	 * <p>
	 * The format of the "//" file itself is simply a list of the long filenames,
	 * each separated by one or more LF characters. Note that the decimal offsets
