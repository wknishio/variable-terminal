 ...
 * 
 * The old ASCII format is limited to 18 bits for the user id, group id, device,
 * and inode numbers. It is limited to 8 gigabyte file sizes.
 * 
 ...
 * 
 * <p>
 * Each file has a 76 (ascii) / 26 (binary) byte header, a variable length, NUL
 ...
 * 
 * <p>
 * If necessary, the filename and file data are padded with a NUL byte to an
 * even length
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * In the ASCII version of this format, the 16-bit entries are represented as
 * 6-byte octal numbers, and the 32-bit entries are represented as 11-byte octal
 ...
 * 
 * <p>
 * Each file has a 110 byte header, a variable length, NUL terminated filename,
 * and variable length file data. A header for a filename "TRAILER!!!" indicates
 * the end of the archive. All the fields in the header are ISO 646
 * (approximately ASCII) strings of hexadecimal numbers, left padded, not NUL
 * terminated.
 * </p>
 * 
 * <pre>
 ...
 * 
 * <p>
 * New ASCII Format The "new" ASCII format uses 8-byte hexadecimal fields for
 * all numbers and separates device numbers into separate fields for major and
 * minor numbers.
 * </p>
 * 
 * <p>
 * The pathname is followed by NUL bytes so that the total size of the fixed
 * header plus pathname is a multiple of four. Likewise, the file data is padded
 ...
 * @NotThreadSafe
 * @see <a href=
 *      "http://people.freebsd.org/~kientzle/libarchive/man/cpio.5.txt">http://people.freebsd.org/~kientzle/libarchive/man/cpio.5.txt</a>
 */
public class CpioArchiveEntry implements CpioConstants, ArchiveEntry {
	...
	/**
	 * The boundary to which the header and data elements are aligned: 0, 2 or 4
	 * bytes
	 */
	...
	/**
	 * Creates a CPIOArchiveEntry with a specified name for a specified file. The
	 * format of this entry will be the new format.
	 * 
	 * @param inputFile
	 ...
	 * @since 1.1
	 */
	public CpioArchiveEntry(final short format, File inputFile, String entryName) {
	...
	/**
	 * Set the checksum. The checksum is calculated by adding all bytes of a file to
	 * transfer (crc += buf[pos] &amp; 0xFF).
	 * 
	 * @param chksum
	 *            The checksum to set.
	 */
	...
	public void setMode(final long mode) {
		...
		default:
			throw new IllegalArgumentException(
					"Unknown mode. " + "Full: " + Long.toHexString(mode) + " Masked: " + Long.toHexString(maskedMode));
		}
