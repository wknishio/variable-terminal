 ...
 * 
 * <pre>
 * CPIOArchiveInputStream cpioIn = new CPIOArchiveInputStream(new FileInputStream(new File(&quot;test.cpio&quot;)));
...
public class CpioArchiveInputStream extends ArchiveInputStream implements CpioConstants {
	 ...
	 * {@link CpioConstants#BLOCK_SIZE BLOCK_SIZE} and expecting ASCII file names.
	 * 
	 * @param in
	 *            The cpio stream
	 */
	...
	/**
	 * Construct the cpio input stream with a blocksize of
	 * {@link CpioConstants#BLOCK_SIZE BLOCK_SIZE}.
	 * 
	 * @param in
	 *            The cpio stream
	 * @param encoding
	 *            The encoding of file names to expect - use null for the platform's
	 *            default.
	 * @since 1.6
	 */
	...
	/**
	 * Construct the cpio input stream with a blocksize of
	 * {@link CpioConstants#BLOCK_SIZE BLOCK_SIZE} expecting ASCII file names.
	 * 
	 * @param in
	 *            The cpio stream
	 * @param blockSize
	...
	/**
	 * Closes the current CPIO entry and positions the stream for reading the next
	 * entry.
	 * 
	 * @throws IOException
	 *             if an I/O error has occurred or if a CPIO file error has occurred
	...
	/**
	 * Reads the next CPIO file entry and positions stream at the beginning of the
	 * entry data.
	 * 
	...
	/**
	 * Reads from the current CPIO entry into an array of bytes. Blocks until some
	 * input is available.
	 * 
	 * @param b
	...
	private CpioArchiveEntry readNewEntry(final boolean hasCrc) throws IOException {
		...
		if (CpioUtil.fileType(mode) == 0 && !name.equals(CPIO_TRAILER)) {
			throw new IOException("Mode 0 only allowed in the trailer. Found entry name: " + name + " Occured at byte: "
					+ getBytesRead());
		}
	...
	private CpioArchiveEntry readOldAsciiEntry() throws IOException {
		...
		if (CpioUtil.fileType(mode) == 0 && !name.equals(CPIO_TRAILER)) {
			throw new IOException(
					"Mode 0 only allowed in the trailer. Found entry: " + name + " Occured at byte: " + getBytesRead());
		}
