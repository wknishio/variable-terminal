 ...
 * {@link #read(byte[], int, int)} - or {@link #read()} - method so that reading
 * from the stream generates EOF for the end of data in each entry as well as at
 * the end of the file proper.
 * <p>
 * The {@link #getNextEntry()} method is used to reset the input stream ready
 * for reading the data from the next entry.
 * <p>
 ...
 * </pre>
 * 
 * which is used by the {@link ArchiveStreamFactory} to autodetect the archive
 * type from the first few bytes of a stream.
 */
public abstract class ArchiveInputStream extends InputStream {
	...
	/*
	 * Note that subclasses also implement specific get() methods which return the
	 * appropriate class without need for a cast. See SVN revision r743259
	 * 
	 * @return
	 * 
	 * @throws IOException
