...
public class TarArchiveEntry implements ArchiveEntry, TarConstants {
	 ...
	 * the entry's header "by hand". File is set to null.
	 *
	 * @param name
	 *            the entry name
	 */
	...
	/**
	 * Set this entry's modification time. The parameter passed to this method is in
	 * "Java time".
	 *
	 * @param time
	 ...
	 *            fields if their value doesn't fit in the maximum size of standard
	 *            tar archives
	 * @since 1.4
	 * @throws IOException
	 *             on error
	 */
	public void writeEntryHeader(final byte[] outbuf, final ZipEncoding encoding, final boolean starMode)
			throws IOException {
		int offset = 0;
	...
	private void parseTarHeader(final byte[] header, final ZipEncoding encoding, final boolean oldStyle)
			throws IOException {
		int offset = 0;
		...
		case FORMAT_POSIX:
		default: {
			final String prefix = oldStyle ? TarUtils.parseName(header, offset, PREFIXLEN)
