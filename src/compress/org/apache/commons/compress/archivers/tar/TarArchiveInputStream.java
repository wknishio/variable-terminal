...
public class TarArchiveInputStream extends ArchiveInputStream {
	...
	public TarArchiveEntry getNextTarEntry() throws IOException {
		...
		try {
			...
			throw new IOException("Error detected parsing the header");
		}
	...
	private void applyPaxHeadersToCurrentEntry(final Map<String, String> headers) {
		/*
		 * The following headers are defined for Pax. atime, ctime, charset: cannot use
		 * these without changing TarArchiveEntry fields mtime comment gid, gname
		 * linkpath size uid,uname SCHILY.devminor, SCHILY.devmajor: don't have
		 * setters/getters for those
		 *
		 * GNU sparse files use additional members, we use GNU.sparse.size to detect the
		 * 0.0 and 0.1 versions and GNU.sparse.realsize for 1.0.
		 *
		 * star files use additional members of which we use SCHILY.filetype in order to
	...
	/**
	 * Adds the sparse chunks from the current entry to the sparse chunks, including
	 * any additional sparse entries following the current entry.
	 *
	 * @throws IOException
	 *             on error
	 *
	...
	private void readOldGNUSparse() throws IOException {
		...
		if (currEntry.isExtended()) {
			...
			do {
				...
				/*
				 * we do not really process sparse files yet sparses.addAll(entry.getSparses());
				 */
	...
	public static boolean matches(final byte[] signature, final int length) {
		...
		if (ArchiveUtils.matchAsciiBuffer(TarConstants.MAGIC_POSIX, signature, TarConstants.MAGIC_OFFSET,
				TarConstants.MAGICLEN)
				&& ArchiveUtils.matchAsciiBuffer(TarConstants.VERSION_POSIX, signature, TarConstants.VERSION_OFFSET,
						TarConstants.VERSIONLEN)) {
			return true;
		}
		if (ArchiveUtils.matchAsciiBuffer(TarConstants.MAGIC_GNU, signature, TarConstants.MAGIC_OFFSET,
				TarConstants.MAGICLEN)
		...
		if (ArchiveUtils.matchAsciiBuffer(TarConstants.MAGIC_ANT, signature, TarConstants.MAGIC_OFFSET,
				TarConstants.MAGICLEN)
				&& ArchiveUtils.matchAsciiBuffer(TarConstants.VERSION_ANT, signature, TarConstants.VERSION_OFFSET,
						TarConstants.VERSIONLEN)) {
			return true;
		}
		return false;
