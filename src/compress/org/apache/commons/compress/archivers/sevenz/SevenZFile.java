 ...
 * <p>
 * The 7z file format is a flexible container that can contain many compression
 * and encryption types, but at the moment only only Copy, LZMA, LZMA2, BZIP2,
 ...
 * using NTFS timestamps (100 nanosecond units since 1 January 1601). Hence the
 * official tools recommend against using it for backup purposes on *nix, and
 * recommend .tar.7z or .tar.lzma or .tar.xz instead.
 * <p>
 * Both the header and file contents may be compressed and/or encrypted. With
 * both encrypted, neither file names nor file contents can be read, but the use
 * of encryption isn't plausibly deniable.
 * 
 * @NotThreadSafe
 * @since 1.6
 */
public class SevenZFile implements Closeable {
	 ...
	 *            the file to read
	 * @param password
	 *            optional password if the archive is encrypted - the byte array is
	 *            supposed to be the UTF16-LE encoded representation of the
	 *            password.
	 * @throws IOException
	 ...
	 *
	 * <p>
	 * This method only provides meta-data, the entries can not be used to read the
	 * contents, you still need to process all entries in order using
	 * {@link #getNextEntry} for that.
	 * </p>
	 *
	 * <p>
	 * The content methods are only available for entries that have already been
	 * reached via {@link #getNextEntry}.
	 * </p>
	 *
	...
	private DataInputStream readEncodedHeader(final DataInputStream header, final Archive archive, byte[] password)
			throws IOException {
		readStreamsInfo(header, archive);
	...
	private void readFilesInfo(final DataInput header, final Archive archive) throws IOException {
		...
		while (true) {
			...
			case NID.kDummy: {
				...
				// Compress up to 1.8.1 would throw an exception, now we ignore it (see
				// COMPRESS-287
	...
	private InputStream buildDecoderStack(final Folder folder, final long folderOffset, final int firstPackStreamIndex,
			SevenZArchiveEntry entry) throws IOException {
		file.seek(folderOffset);
		InputStream inputStreamStack = new BufferedInputStream(
				new BoundedRandomAccessFileInputStream(file, archive.packSizes[firstPackStreamIndex]));
