 ...
 *
 * <p>
 * Currently Commons Compress doesn't support encrypting the central directory
 * so the note in APPNOTE.TXT about masking doesn't apply.
 * </p>
 *
 * <p>
 * The implementation relies on data being read from the local file header and
 * assumes that both size values are always present.
 * </p>
 *
...
public class Zip64ExtendedInformationExtraField implements ZipExtraField {
	...
	private static final String LFH_MUST_HAVE_BOTH_SIZES_MSG = "Zip64 extended information must contain"
	 ...
	 * parseFromCentralDirectoryData} so it can be reused when ZipFile calls
	 * {@link #reparseCentralDirectoryData reparseCentralDirectoryData}.
	 *
	 * <p>
	...
	/**
	 * This constructor should only be used by the code that reads archives inside
	 * of Commons Compress.
	 */
	...
	public ZipShort getCentralDirectoryLength() {
		return new ZipShort((size != null ? DWORD : 0) + (compressedSize != null ? DWORD : 0)
				+ (relativeHeaderOffset != null ? DWORD : 0) + (diskStart != null ? WORD : 0));
	}
	...
	/**
	 * Parses the raw bytes read from the central directory extra field with
	 * knowledge which fields are expected to be there.
	 *
	 * <p>
	 * All four fields inside the zip64 extended information extra field are
	 * optional and must only be present if their corresponding entry inside the
	 * central directory contains the correct magic value.
	 * </p>
	 *
	 * @param hasUncompressedSize
	 *            flag to read from central directory
	 * @param hasCompressedSize
	 *            flag to read from central directory
	 * @param hasRelativeHeaderOffset
	 *            flag to read from central directory
	 * @param hasDiskStart
	 *            flag to read from central directory
	 * @throws ZipException
