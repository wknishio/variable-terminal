 ...
 * entries. As of Apache Commons Compress 1.3 it also transparently supports
 * Zip64 extensions and thus individual entries and archives larger than 4 GB or
...
public class ZipFile implements Closeable {
	 ...
	 */
	private final Map<String, LinkedList<ZipArchiveEntry>> nameMap = new HashMap<String, LinkedList<ZipArchiveEntry>>(
			HASH_SIZE);
	 ...
	 *
	 * <p>
	 * Entries will be returned in the same order they appear within the archive's
	 * central directory.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * Entries will be returned in the same order their contents appear within the
	 * archive.
	 * </p>
	 *
	 * @return all entries as {@link ZipArchiveEntry} instances
	 ...
	 *
	 * <p>
	 * If multiple entries with the same name exist the first entry in the archive's
	 * central directory by that name is returned.
	 * </p>
	 *
	 * @param name
	 *            name of the entry.
	 * @return the ZipArchiveEntry corresponding to the given name - or {@code null}
	 *         if not present.
	 */
	...
	/**
	 * Returns all named entries in the same order they appear within the archive's
	 * central directory.
	 *
	 * @param name
	 *            name of the entry.
	...
	/**
	 * Returns all named entries in the same order their contents appear within the
	 * archive.
	 *
	 * @param name
	 *            name of the entry.
	...
	/**
	 * <p>
	 * Convenience method to return the entry's content as a String if
	 * isUnixSymlink() returns true for it, otherwise returns null.
	 * </p>
	 *
	 * <p>
	 * This method assumes the symbolic link's file name uses the same encoding that
	 * as been specified for this ZipFile.
	 * </p>
	 *
	 * @param entry
	...
	/**
	 * Ensures that the close method of this zipfile is called when there are no
	...
	/**
	 * Length of a "central directory" entry structure without file name, extra
	 * fields or comment.
	 */
	 ...
	 * tables with ZipArchiveEntry instances.
	 *
	 * <p>
	 * The ZipArchiveEntrys will know all data that can be obtained from the central
	 * directory alone, but not the data that requires the local file header or
	 * additional data to be read.
	 * </p>
	 *
	 * @return a map of zipentries that didn't have the language encoding flag set
	 *         when read.
	 */
	...
	/**
	 * Reads an individual entry of the central directory, creats an ZipArchiveEntry
	 * from it and adds it to the global maps.
	 *
	 * @param noUTF8Flag
	 *            map used to collect entries that don't have their UTF-8 flag set
	 *            and whose name will be set by data read from the local file header
	...
	/**
	 * If the entry holds a Zip64 extended information extra field, read sizes from
	 * there if the entry's sizes are set to 0xFFFFFFFFF, do the same for the offset
	 * of the local file header.
	 *
	 * <p>
	 * Ensures the Zip64 extra either knows both compressed and uncompressed size or
	 * neither of both as the internal logic in ExtraFieldUtils forces the field to
	 * create local header data even if they are never used - and here a field with
	 * only one size would be invalid.
	 * </p>
	 */
	...
	/**
	 * Length of the "End of central directory record" - which is supposed to be the
	 * last structure of the archive - without file comment.
	 */
	static final int MIN_EOCD_SIZE = /* end of central dir signature */ WORD/* number of this disk */ + SHORT
	 ...
	 */
	private static final int MAX_EOCD_SIZE = MIN_EOCD_SIZE/* maximum length of zipfile comment */ + ZIP64_MAGIC_SHORT;

	/**
	 * Offset of the field that holds the location of the first central directory
	 * entry inside the "End of central directory record" relative to the start of
	...
	/**
	 * Length of the "Zip64 end of central directory locator" - which should be
	 * right in front of the "end of central directory record" if one is present at
	 * all.
	 */
	...
	/**
	 * Offset of the field that holds the location of the "Zip64 end of central
	 * directory record" inside the "Zip64 end of central directory locator"
	 * relative to the start of the "Zip64 end of central directory locator".
	 */
	...
	/**
	 * Offset of the field that holds the location of the first central directory
	 * entry inside the "Zip64 end of central directory record" relative to the
	...
	/**
	 * Searches for either the &quot;Zip64 end of central directory locator&quot; or
	 * the &quot;End of central dir record&quot;, parses it and positions the stream
	...
	private void positionAtCentralDirectory() throws IOException {
		...
		if (searchedForZip64EOCD) {
			...
			archive.readFully(WORD_BUF);
			found = Arrays.equals(ZipArchiveOutputStream.ZIP64_EOCD_LOC_SIG, WORD_BUF);
		}
		if (!found) {
			// not a ZIP64 archive
			if (searchedForZip64EOCD) {
	...
	/**
	 * Parses the &quot;Zip64 end of central directory locator&quot;, finds the
	 * &quot;Zip64 end of central directory record&quot; using the parsed
	 * information, parses that and positions the stream at the first central
	 * directory record.
	 *
	 * Expects stream to be positioned right behind the &quot;Zip64 end of central
	 * directory locator&quot;'s signature.
	 */
	...
	/**
	 * Parses the &quot;End of central dir record&quot; and positions the stream at
	 * the first central directory record.
	 *
	 * Expects stream to be positioned at the beginning of the &quot;End of central
	 * dir record&quot;.
	 */
	...
	/**
	 * Searches for the and positions the stream at the start of the &quot;End of
	 * central dir record&quot;.
	 */
	private void positionAtEndOfCentralDirectoryRecord() throws IOException {
		boolean found = tryToLocateSignature(MIN_EOCD_SIZE, MAX_EOCD_SIZE, ZipArchiveOutputStream.EOCD_SIG);
		if (!found) {
	...
	/**
	 * Searches the archive backwards from minDistance to maxDistance for the given
	 * signature, positions the RandomaccessFile right at the signature if it has
	 * been found.
	 */
	 ...
	 *
	 * <p>
	 * Won't return any meaningful results if one of the entries isn't part of the
	 * archive at all.
	 * </p>
	 *
	 * @since 1.1
	...
	private static class Entry extends ZipArchiveEntry {
		...
		public int hashCode() {
			return 3 * super.hashCode() + (int) (offsetEntry.headerOffset % Integer.MAX_VALUE);
		}
		...
		public boolean equals(Object other) {
			if (super.equals(other)) {
				...
				return offsetEntry.headerOffset == otherEntry.offsetEntry.headerOffset
						&& offsetEntry.dataOffset == otherEntry.offsetEntry.dataOffset;
			}
			return false;
		}
	}
