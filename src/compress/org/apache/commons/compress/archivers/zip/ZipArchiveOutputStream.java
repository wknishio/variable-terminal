 ...
 * {@link #putArchiveEntry(ArchiveEntry)} can be called.
 * </p>
 *
 * <p>
 * As of Apache Commons Compress 1.3 it transparently supports Zip64 extensions
 * and thus individual entries and archives larger than 4 GB or with more than
 * 65536 entries in most cases but explicit control is provided via
 * {@link #setUseZip64}. If the stream can not user RandomAccessFile and you try
 * to write a ZipArchiveEntry of unknown size then Zip64 extensions will be
 * disabled by default.
 * </p>
 *
 * @NotThreadSafe
...
public class ZipArchiveOutputStream extends ArchiveOutputStream {
	 ...
	 * size for {@link #STORED} entries before invoking
	 * {@link #putArchiveEntry(ArchiveEntry)}.
	 * 
	 ...
	 *
	 * @param b
	 *            whether to fall back to UTF and the language encoding flag if the
	 *            file name cannot be encoded using the specified encoding.
	 */
	 ...
	 * {@link #putArchiveEntry}, {@link #closeArchiveEntry}, {@link #finish} or
	 * {@link #close} may throw a {@link Zip64RequiredException} if the entry's size
	 * or the total size of the archive exceeds 4GB or there are more than 65536
	 * entries inside the archive. Any archive created in this mode will be readable
	 * by implementations that don't support Zip64.
	 * </p>
	 *
	 * <p>
	 * When setting the mode to {@link Zip64Mode#Always Always}, Zip64 extensions
	 * will be used for all entries. Any archive created in this mode may be
	 * unreadable by implementations that don't support Zip64 even if all its
	 * contents would be.
	 * </p>
	 *
	 * <p>
	 * When setting the mode to {@link Zip64Mode#AsNeeded AsNeeded}, Zip64
	 * extensions will transparently be used for those entries that require them.
	 * This mode can only be used if the uncompressed size of the
	 * {@link ZipArchiveEntry} is known when calling {@link #putArchiveEntry} or the
	 * archive is written to a seekable output (i.e. you have used the
	 * {@link #ZipArchiveOutputStream(java.io.File) File-arg constructor}) - this
	 * mode is not valid when the output stream is not seekable and the uncompressed
	 * size is unknown when {@link #putArchiveEntry} is called.
	 * </p>
	 * 
	 * <p>
	 ...
	 * {@link Zip64Mode#AsNeeded AsNeeded} will create a slightly bigger archive if
	 * the uncompressed size of any entry has initially been unknown and create an
	 * archive identical to {@link Zip64Mode#Never Never} otherwise.
	 * {@link Zip64Mode#Always Always} will create an archive that is at least 24
	 * bytes per entry bigger than the one {@link Zip64Mode#Never Never} would
	 * create.
	 * </p>
	 *
	 * <p>
	 * Defaults to {@link Zip64Mode#AsNeeded AsNeeded} unless
	 * {@link #putArchiveEntry} is called with an entry of unknown size and data is
	 ...
	 *             on error
	 * @throws Zip64RequiredException
	 *             if the entry's uncompressed or compressed size exceeds 4 GByte
	 ...
	 *             and {@link #setUseZip64} is {@link Zip64Mode#Never}.
	 * @param phased
	 *            This entry is second phase of a 2-phase zip creation, size,
	 *            compressed size and crc are known in ZipArchiveEntry
	 */
	 ...
	 *
	 * If crc, size and compressed size are supplied on the entry, these values will
	 * be used as-is. Zip64 status is re-established based on the settings in this
	 ...
	 * @param rawStream
	 *            The raw input stream of a different entry. May be
	...
	public void addRawArchiveEntry(ZipArchiveEntry entry, InputStream rawStream) throws IOException {
		...
		if (hasZip64Extra(ae)) {
			// Will be re-added as required. this may make the file generated with this
			// method
		...
		boolean is2PhaseSource = ae.getCrc() != ZipArchiveEntry.CRC_UNKNOWN && ae.getSize() != ArchiveEntry.SIZE_UNKNOWN
	...
	private boolean handleSizesAndCrc(long bytesWritten, long crc, Zip64Mode effectiveMode) throws ZipException {
		if (entry.entry.getMethod() == DEFLATED) {
			/*
			 * It turns out def.getBytesRead() returns wrong values if the size exceeds 4 GB
			 * on Java < Java7 entry.entry.setSize(def.getBytesRead());
			 */
	...
	/**
	 * When using random access output, write the local file header and potentiall
	 * the ZIP64 extra containing the correct CRC and compressed/uncompressed sizes.
	 */
	private void rewriteSizesAndCrc(boolean actuallyNeedsZip64) throws IOException {
		...
		if (hasZip64Extra(entry.entry)) {
			...
			raf.seek(entry.localDataStart + 3 * WORD + 2 * SHORT + nameLen + 2 * SHORT);
	 ...
	 * @throws Zip64RequiredException
	 *             if the entry's uncompressed or compressed size is known to exceed
	 *             4 GByte and {@link #setUseZip64} is {@link Zip64Mode#Never}.
	 */
	...
	/**
	 * Writes the headers for an archive entry to the output stream. The caller must
	 * then write the content to the stream and call {@link #closeArchiveEntry()} to
	 * complete the process.
	 * 
	 * @param archiveEntry
	 *            The archiveEntry
	 * @param phased
	 *            If true size, compressedSize and crc required to be known up-front
	 *            in the archiveEntry
	 * @throws ClassCastException
	 *             if entry is not an instance of ZipArchiveEntry
	 * @throws Zip64RequiredException
	...
	/**
	 * Throws an exception if the size is unknown for a stored entry that is written
	 * to a non-seekable output or the entry is too big to be written without Zip64
	 * extra but the mode has been set to Never.
	 */
	...
	/**
	 * Whether to addd a Zip64 extended information extra field to the local file
	 * header.
	 *
	 * <p>
	 * Returns true if
	 ...
	 *
	 * <p>
	 * May return false if it is set up to use encryption or a compression method
	...
	public boolean canWriteEntryData(ArchiveEntry ae) {
		if (ae instanceof ZipArchiveEntry) {
			...
			return zae.getMethod() != ZipMethod.IMPLODING.getCode()
					&& zae.getMethod() != ZipMethod.UNSHRINKING.getCode() && ZipUtil.canHandleEntryData(zae);
		}
		return false;
	}
	 ...
	 * data cannot be encoded using the configured encoding.
	 */
	...
	private byte[] createCentralFileHeader(ZipArchiveEntry ze) throws IOException {
		...
		final boolean needsZip64Extra = hasZip64Extra(ze) || ze.getCompressedSize() >= ZIP64_MAGIC
				|| ze.getSize() >= ZIP64_MAGIC || lfhOffset >= ZIP64_MAGIC || zip64Mode == Zip64Mode.Always;
	 ...
	 *            The encoded name
	 * @param lfhOffset
	 *            Local file header offset for this file
	 * @throws IOException
	 *             on error
	 */
	private byte[] createCentralFileHeader(ZipArchiveEntry ze, ByteBuffer name, long lfhOffset, boolean needsZip64Extra)
		...
		putShort((ze.getPlatform() << 8) | (!hasUsedZip64 ? DATA_DESCRIPTOR_MIN_VERSION : ZIP64_MIN_VERSION), buf,
	...
	/**
	 * Writes the &quot;ZIP64 End of central dir record&quot; and &quot;ZIP64 End of
	 * central dir locator&quot;.
	 * 
	 * @throws IOException
	 *             on error
	...
	protected void writeZip64CentralDirectory() throws IOException {
		...
		if (!hasUsedZip64
				&& (cdOffset >= ZIP64_MAGIC || cdLength >= ZIP64_MAGIC || entries.size() >= ZIP64_MAGIC_SHORT)) {
			// actually "will use"
			hasUsedZip64 = true;
		}
	...
	private int versionNeededToExtract(final int zipMethod, final boolean zip64) {
		...
		return (isDeflatedToOutputStream(zipMethod)) ? DATA_DESCRIPTOR_MIN_VERSION : INITIAL_VERSION;
	}
	...
	/**
	 * Creates a new zip entry taking some information from the given file and using
	 * the provided name.
	 *
	 * <p>
	 * The name will be adjusted to end with a forward slash "/" if the file is a
	 * directory. If the file is not a directory a potential trailing forward slash
	...
	/**
	 * Get the existing ZIP64 extended information extra field or create a new one
	 * and add it to the entry.
	 *
	 * @since 1.3
	 */
	private Zip64ExtendedInformationExtraField getZip64Extra(ZipArchiveEntry ze) {
		...
		if (z64 == null) {
			/*
			 * System.err.println("Adding z64 for " + ze.getName() + ", method: " +
			 * ze.getMethod() + " (" + (ze.getMethod() == STORED) + ")" + ", raf: " + (raf
			 * != null));
			 */
	...
	/**
	 * If the mode is AsNeeded and the entry is a compressed entry of unknown size
	 * that gets written to a non-seekable stream the change the default to Never.
	 *
	 * @since 1.3
	 */
	private Zip64Mode getEffectiveZip64Mode(ZipArchiveEntry ze) {
		if (zip64Mode != Zip64Mode.AsNeeded || raf != null || ze.getMethod() != DEFLATED
	...
	/**
	 * Closes the underlying stream/file without finishing the archive, the result
	 * will likely be a corrupt archive.
	 *
	 * <p>
	 * This method only exists to support tests that generate corrupt archives so
	 * they can clean up any temporary files.
	 * </p>
	 */
	...
	private static final class CurrentEntry {
		 ...
		 *
		 * <p>
		 * In order to create a valid archive {@link #closeArchiveEntry
		 * closeArchiveEntry} will write an empty array to get the CRC right if nothing
		 * has been written to the stream at all.
		 * </p>
		 */
