...
/**
 * NTFS extra field that was thought to store various attributes but in reality
...
public class X000A_NTFS implements ZipExtraField {
	 ...
	 * central cannot contain access or create timestamps.
	 * </p>
	 *
	 * @return a <code>ZipShort</code> for the length of the data of this extra
	 *         field
	...
	/**
	 * The actual data to put into central directory data - without Header-ID or
	 * length specifier.
	 *
	...
	/**
	 * Doesn't do anything special since this class always uses the same parsing
	 * logic for both central directory and local file data.
	 */
	...
	/**
	 * Returns the "File last modification time" of this zip entry as a
	 * ZipEightByteInteger object, or {@link ZipEightByteInteger#ZERO} if no such
	 * timestamp exists in the zip entry.
	 *
	...
	/**
	 * Returns the "File creation time" of this zip entry as a ZipEightByteInteger
	 * object, or {@link ZipEightByteInteger#ZERO} if no such timestamp exists in
	 * the zip entry.
	 *
	...
	/**
	 * Returns the modify time as a java.util.Date of this zip entry, or null if no
	...
	/**
	 * Returns the access time as a java.util.Date of this zip entry, or null if no
	 * such timestamp exists in the zip entry.
	 *
	...
	/**
	 * Returns the create time as a a java.util.Date of this zip entry, or null if
	 * no such timestamp exists in the zip entry.
	 *
	...
	/**
	 * Sets the File last access time of this zip entry using a ZipEightByteInteger
	 * object.
	 *
	 * @param t
	...
	/**
	 * Sets the File creation time of this zip entry using a ZipEightByteInteger
	 * object.
	 *
	 * @param t
	...
	/**
	 * <p>
	 * Sets the create time as a java.util.Date of this zip entry. Supplied value is
	 * truncated to per-second precision (milliseconds zeroed-out).
	 * </p>
	 * <p>
	 * Note: the setters for flags and timestamps are decoupled. Even if the
	 * timestamp is not-null, it will only be written out if the corresponding bit
	 * in the flags is also set.
	 * </p>
	 *
	 * @param d
	...
	public String toString() {
		...
		buf.append("0x000A Zip Extra Field:").append(" Modify:[").append(getModifyJavaTime()).append("] ")
				.append(" Access:[").append(getAccessJavaTime()).append("] ").append(" Create:[")
				.append(getCreateJavaTime()).append("] ");
		return buf.toString();
	}
	...
	/**
	 * Reset state back to newly constructed state. Helps us make sure parse() calls
	 * always generate clean results.
	 */
	private void reset() {
