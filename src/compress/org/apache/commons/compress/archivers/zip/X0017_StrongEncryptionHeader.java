 ...
 * <li>0x0100 - Indicates non-OAEP key wrapping was used. If this this field is
 * set, the version needed to extract must be at least 61. This means OAEP key
 ...
 * details. Refer to the section in this document entitled "Incorporating PKWARE
...
public class X0017_StrongEncryptionHeader extends PKWareExtraHeader implements ZipExtraField {
		...
		if (rcount > 0) {
			...
			for (int i = 0; i < this.rcount; i++) {
				for (int j = 0; j < this.hashSize; j++) {
					// ZipUtil.signedByteToUnsignedInt(data[offset + 16 + (i * this.hashSize) +
					// j]));
				}
			}
		}
	...
	public void parseFileFormat(byte[] data, int offset, int length) {
		...
		if (rcount == 0) {
			...
			System.arraycopy(data, offset + ivSize + 24 + erdSize + this.hashSize, this.keyBlob, 0,
					resize - this.hashSize);
