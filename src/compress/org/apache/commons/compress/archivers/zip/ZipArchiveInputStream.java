 ...
 *
 * <p>
 * As of Apache Commons Compress it transparently supports Zip64 extensions and
 * thus individual entries and archives larger than 4 GB or with more than 65536
 * entries.
 * </p>
 *
 * <p>
 * The {@link ZipFile} class is preferred when reading from files as
 * {@link ZipArchiveInputStream} is limited by not being able to read the
 * central directory header before returning entries. In particular
 * {@link ZipArchiveInputStream}
 * </p>
 *
 * <ul>
 *
 * <li>may return entries that are not part of the central directory at all and
 * shouldn't be considered part of the archive.</li>
 *
 ...
 *
 * <li>may return unknown sizes and CRC values for entries until the next entry
 * has been reached if the archive uses the data descriptor feature.</li>
 *
 * </ul>
 *
 * @see ZipFile
...
public class ZipArchiveInputStream extends ArchiveInputStream {
	...
	/**
	 * Whether the stream has reached the central directory - and thus found all
	 * entries.
	 */
	private boolean hitCentralDirectory = false;

	/**
	 * When reading a stored entry that uses the data descriptor this stream has to
	 * read the full entry and caches it. This is the cache.
	 */
	...
	/*
	 * local file header signature WORD version needed to extract SHORT general
	 * purpose bit flag SHORT compression method SHORT last mod file time SHORT last
	 * mod file date SHORT crc-32 WORD compressed size WORD uncompressed size WORD
	 * file name length SHORT extra field length SHORT
	 */

	private static final int CFH_LEN = 46;
	/*
	 * central file header signature WORD version made by SHORT version needed to
	 * extract SHORT general purpose bit flag SHORT compression method SHORT last
	 * mod file time SHORT last mod file date SHORT crc-32 WORD compressed size WORD
	 * uncompressed size WORD file name length SHORT extra field length SHORT file
	 * comment length SHORT disk number start SHORT internal file attributes SHORT
	 * external file attributes WORD relative offset of local header WORD
	 */
	...
	public ZipArchiveEntry getNextZipEntry() throws IOException {
		...
		if (current.entry.getCompressedSize() != ZipArchiveEntry.SIZE_UNKNOWN) {
			if (current.entry.getMethod() == ZipMethod.UNSHRINKING.getCode()) {
				...
				current.in = new ExplodingInputStream(current.entry.getGeneralPurposeBit().getSlidingDictionarySize(),
	...
	/**
	 * Records whether a Zip64 extra is present and sets the size information from
	 * it if sizes are 0xFFFFFFFF and the entry doesn't use a data descriptor.
	 */
	private void processZip64Extra(ZipLong size, ZipLong cSize) {
		Zip64ExtendedInformationExtraField z64 = (Zip64ExtendedInformationExtraField) current.entry
	 ...
	 *
	 * <p>
	 * This implementation may end up skipping over some smaller number of bytes,
	 * possibly 0, if and only if it reaches the end of the underlying stream.
	 * </p>
	 *
	 * <p>
	...
	/**
	 * Checks if the signature matches what is expected for a zip file. Does not
	 * currently handle self-extracting zips which may have arbitrary leading
	 * content.
	 *
	 * @param signature
	 *            the bytes to check
	...
	/**
	 * Closes the current ZIP archive entry and positions the underlying stream to
	 * the beginning of the next entry. All per-entry variables and data structures
	 * are cleared.
	 * <p>
	 * If the compressed size of this entry is included in the entry header, then
	 * any outstanding bytes are simply skipped from the underlying stream without
	 * uncompressing them. This allows an entry to be safely closed even if the
	 * compression method is unsupported.
	 * <p>
	 * In case we don't know the compressed size of this entry or have already
	 * buffered too much data from the underlying stream to support uncompression,
	 * then the uncompression process is completed and the end position of the
	 * stream is adjusted based on the result of that process.
	 *
	 * @throws IOException
	 *             if an error occurs
	 */
	private void closeEntry() throws IOException {
		...
		if (current.bytesReadFromStream <= current.entry.getCompressedSize() && !current.hasDataDescriptor) {
			...
			long inB = current.entry.getMethod() == ZipArchiveOutputStream.DEFLATED ? getBytesInflated()
					: current.bytesRead;
	...
	/**
	 * Read all data of the current entry from the underlying stream that hasn't
	 * been read, yet.
	 */
	 ...
	 *
	 * <p>
	 * for Java &lt; Java7 the getBytes* methods in Inflater/Deflater seem to return
	 * unsigned ints rather than longs that start over with 0 at 2^32.
	 * </p>
	 *
	 * <p>
	 * The stream knows how many bytes it has read, but not how many the Inflater
	 * actually consumed - it should be between the total number of bytes read for
	 * the entry and the total number minus the last read operation. Here we just
	 * try to make the value close enough to the bytes we've read by assuming the
	 * number of bytes consumed must be smaller than (or equal to) the number of
	 ...
	 *
	 * @return true if allowStoredEntriesWithDataDescriptor is true, the entry
	 *         doesn't require any data descriptor or the method is DEFLATED.
	 */
	 ...
	 *
	 * <ul>
	 * <li>Reads a stored entry until the signature of a local file header, central
	 * directory header or data descriptor has been found.</li>
	 ...
	 *
	 * <p>
	 * After calling this method the entry should know its size, the entry's data is
	 * cached and the stream is positioned at the next local file or central
	 * directory header.
	 * </p>
	 */
	...
	/**
	 * Checks whether the current buffer contains the signature of a &quot;data
	 * descriptor&quot;, &quot;local file header&quot; or &quot;central directory
	 * entry&quot;.
	 *
	 * <p>
	 * If it contains such a signature, reads the data descriptor and positions the
	 * stream right after the data descriptor.
	 * </p>
	 */
	...
	/**
	 * If the last read bytes could hold a data descriptor and an incomplete
	 * signature then save the last bytes to the front of the buffer and cache
	 * everything in front of the potential data descriptor into the given
	 * ByteArrayOutputStream.
	 *
	 * <p>
	 * Data descriptor plus incomplete signature (3 bytes in the worst case) can be
	 * 20 bytes max.
	 * </p>
	 */
	...
	/**
	 * Reads the stream until it find the "End of central directory record" and
	 * consumes it as well.
	 */
	...
	/**
	 * Reads forward until the signature of the &quot;End of central directory&quot;
	 * record is found.
	 */
	...
	/**
	 * Skips bytes by reading from the underlying stream rather than the
	 * (potentially inflating) archive stream - which {@link #skip} would do.
	 *
	...
	/**
	 * Reads bytes by reading from the underlying stream rather than the
	 * (potentially inflating) archive stream - which {@link #read} would do.
	 *
	 * Also updates bytes-read counter.
	 */
	...
	private static final class CurrentEntry {
		 ...
		 *
		 * <p>
		 * This may be more than the actual entry's length as some stuff gets buffered
		 * up and needs to be pushed back when the end of the entry has been reached.
		 * </p>
		 */
