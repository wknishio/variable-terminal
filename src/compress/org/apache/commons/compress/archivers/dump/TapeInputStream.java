...
/**
 * Filter stream that mimics a physical tape drive capable of compressing the
 * data stream.
 *
 * @NotThreadSafe
 */
class TapeInputStream extends FilterInputStream {
	...
	/**
	 * Set the DumpArchive Buffer's block size. We need to sync the block size with
	 * the dump archive's actual block size since compression is handled at the
	 * block level.
	 *
	 * @param recsPerBlock
	 *            records per block
	...
	public int read() throws IOException {
		throw new IllegalArgumentException("all reads must be multiple of record size (" + recordSize + " bytes.");
	}
	 ...
	 *
	 * @param len
	 *            length to read, must be a multiple of the stream's record size
	 */
	...
	public long skip(long len) throws IOException {
		...
		while (bytes < len) {
			...
			// return -1 if there's a problem.
			if ((readOffset == blockSize) && !readBlock((len - bytes) < blockSize)) {
				return -1;
			}
	 ...
	 *
	 * @param decompress
	 *            if false the buffer will not be decompressed. This is an
	 *            optimization for longer seeks.
