 ...
 * system it seems very likely the format uses the current default encoding of
 * the system.
 *
 * @NotThreadSafe
 */
public class DumpArchiveInputStream extends ArchiveInputStream {
	...
	// map of ino -> (directory) entry when we're missing one or more elements in
	// the path.
	...
	private void readCLRI() throws IOException {
		...
		if (raw.skip(DumpArchiveConstants.TP_SIZE * active.getHeaderCount()) == -1) {
	...
	public DumpArchiveEntry getNextEntry() throws IOException {
		...
		while (entry == null) {
			...
			while (DumpArchiveConstants.SEGMENT_TYPE.ADDR == active.getHeaderType()) {
				if (raw.skip(
						DumpArchiveConstants.TP_SIZE * (active.getHeaderCount() - active.getHeaderHoles())) == -1) {
					throw new EOFException();
				}
	...
	private void readDirectoryEntry(DumpArchiveEntry entry) throws IOException {
		...
		while (first || DumpArchiveConstants.SEGMENT_TYPE.ADDR == entry.getHeaderType()) {
			...
			for (int i = 0; i < datalen - 8 && i < size - 8; i += reclen) {
				 ...
				 * System.out.println("we already have ino: " + names.get(ino)); }
				 */
	 ...
	 * With 32 bytes we can look at the magic value, with a full 1k we can verify
	 * the checksum.
	 * 
	 * @param buffer
	 *            data to match
