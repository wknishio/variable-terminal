 ...
 * <p>
 * <a href=
 * "http://farmanager.com/svn/trunk/plugins/multiarc/arc.doc/arj.txt">Reference</a>
 * 
 * @NotThreadSafe
 * @since 1.6
 */
public class ArjArchiveInputStream extends ArchiveInputStream {
	...
	/**
	 * Constructs the ArjInputStream, taking ownership of the inputStream that is
	 * passed in.
	 * 
	 * @param inputStream
	 *            the underlying stream, whose ownership is taken
	 * @param charsetName
	 *            the charset used for file names and comments in the archive. May
	 *            be {@code null} to use the platform default.
	...
	/**
	 * Constructs the ArjInputStream, taking ownership of the inputStream that is
	 * passed in, and using the CP437 character encoding.
	 * 
	 * @param inputStream
	 *            the underlying stream, whose ownership is taken
	 * @throws ArchiveException
	...
	private String readString(final DataInputStream dataIn) throws IOException {
		...
		if (charsetName != null) {
			...
			// intentionally using the default encoding as that's the contract for a null
			// charsetName
			return new String(buffer.toByteArray());
		}
	}

	private void readFully(final DataInputStream dataIn, byte[] b) throws IOException {
		dataIn.readFully(b);
		count(b.length);
	}
	...
	private void readExtraData(int firstHeaderSize, DataInputStream firstHeader, LocalFileHeader localFileHeader)
			throws IOException {
		if (firstHeaderSize >= 33) {
