...
/**
 * ChangeSet collects and performs changes to an archive. Putting delete changes
 * in this ChangeSet from multiple threads can cause conflicts.
 * 
 * @NotThreadSafe
 */
public final class ChangeSet {
	...
	/**
	 * Adds a new archive entry to the archive. If replace is set to true, this
	 * change will replace all other additions done in this ChangeSet and all
	 * existing entries in the original stream.
	 * 
	 * @param pEntry
	 *            the entry to add
	 * @param pInput
			...
			for (Iterator<Change> it = changes.iterator(); it.hasNext();) {
				...
				if (change.type() == Change.TYPE_ADD && change.getEntry() != null) {
					...
					if (Change.TYPE_DELETE == pChange.type() && source.equals(target)) {
						it.remove();
					} else if (Change.TYPE_DELETE_DIR == pChange.type() && target.matches(source + "/.*")) {
						it.remove();
					}
				}
			}
	...
	/**
	 * Returns the list of changes as a copy. Changes on this set are not reflected
	 * on this ChangeSet and vice versa.
	 * 
