...
public final class ByteUtils {
	...
	public interface ByteSupplier {
		/**
		 * The contract is similar to {@link InputStream#read()}, return the byte as an
		 * unsigned int, -1 if there are no more bytes.
		 * 
	...
	public interface ByteConsumer {
		/**
		 * The contract is similar to {@link OutputStream#write(int)}, consume the lower
		 * eight bytes of the int as a byte.
		 * 
		 * @param b
		 *            the byte to consume
		 * @throws IOException
	...
	/**
	 * Reads the given number of bytes from the given stream as a little endian
	 * long.
	 * 
	 * @param in
	 ...
	 *             if len is bigger than eight
	 * @throws IOException
	 *             if reading fails or the stream doesn't contain the given number
	 *             of bytes anymore
	 */
	public static long fromLittleEndian(InputStream in, int length) throws IOException {
		// somewhat duplicates the ByteSupplier version in order to save the creation of
		// a wrapper object
		checkReadLength(length);
		long l = 0;
		for (int i = 0; i < length; i++) {
	...
	/**
	 * Reads the given number of bytes from the given supplier as a little endian
	 * long.
	 *
	 * <p>
	 ...
	 *             if len is bigger than eight
	 * @throws IOException
	 *             if the supplier fails or doesn't supply the given number of bytes
	 *             anymore
	 */
	...
	/**
	 * Inserts the given value into the array as a little endian sequence of the
	 * given length starting at the given offset.
	 * 
	 * @param b
	...
	/**
	 * Writes the given value to the given stream as a little endian array of the
	 * given length.
	 * 
	 * @param out
	...
	public static void toLittleEndian(OutputStream out, final long value, final int length) throws IOException {
		// somewhat duplicates the ByteConsumer version in order to save the creation of
		// a wrapper object
		long num = value;
		for (int i = 0; i < length; i++) {
	...
	/**
	 * Provides the given value to the given consumer as a little endian sequence of
	 * the given length.
	 * 
	 * @param consumer
