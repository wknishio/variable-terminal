...
public class NatPmpDevice {
	 ...
	 *            {@link #setShutdownHookEnabled(boolean)} for details about what
	 *            value should be provided here and how it alters the object's
	 *            behavior.
	 *
	 * @throws NatPmpException
	 *             A NatPmpException may be thrown if the local network is not using
	 *             addresses defined in RFC1918. A NatPmpException may also be
	 *             thrown if the the network gateway cannot be determined, which may
	 *             rarely be due to the network not using IPv4. NAT-PMP should only
	 *             be used on RFC1918 networks using IPv4.
	 *
	...
	/**
	 * Clears the queue of messages to send. If a message is currently sending, it
	 * is not interrupted.
	 * 
	...
	/**
	 * Shuts down this NatPmpDevice. If the shutdown hook is disabled, this method
	 * should be called manually at the time port mappings through the NAT-PMP
	 * gateway are no longer needed. If the shutdown hook is enabled, this method is
	 * called automatically during Java VM shutdown.
	 *
	 * When this method is called, if the shutdown hook is enabled, it is
	 * automatically disabled.
	 *
	 * It should be noted that when this method is called manually, it blocks until
	 * it completes. If it is desired to shutdown asynchronously, the
	...
	/**
	 * Flag indicates if this NatPmpDevice is shutdown. This method will block if a
	 * shutdown is in progress. If you desire to wait for an asynchronous shutdown
	 * to complete, please monitor the returned {@link Thread} instead.
	 * 
