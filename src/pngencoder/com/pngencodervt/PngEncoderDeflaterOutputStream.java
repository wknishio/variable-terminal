...
class PngEncoderDeflaterOutputStream extends FilterOutputStream {
    ...
    static final int COUNT_MAX_QUEUED_TASKS = PngEncoderDeflaterExecutorService.NUM_THREADS_IS_AVAILABLE_PROCESSORS * 1;
    ...
    private final ConcurrentLinkedQueue<PngEncoderDeflaterSegmentTask> resultQueue;
    private PngEncoderDeflaterBuffer originalSegment;
    private long adler32;
    private boolean finished;
    private boolean closed;

    PngEncoderDeflaterOutputStream(OutputStream out, int compressionLevel, int compressionStrategy, int segmentMaxLengthOriginal, PngEncoderDeflaterBufferPool pool) throws IOException {
        ...
        this.resultQueue = new ConcurrentLinkedQueue<PngEncoderDeflaterSegmentTask>();
    ...
    PngEncoderDeflaterOutputStream(OutputStream out, int compressionLevel, int compressionStrategy, int segmentMaxLengthOriginal) throws IOException {
        this(out, compressionLevel, compressionStrategy, segmentMaxLengthOriginal, new PngEncoderDeflaterBufferPool(getSegmentMaxLengthDeflated(segmentMaxLengthOriginal)));
    }
    ...
    void submitTask(boolean lastSegment) {
        ...
        final PngEncoderDeflaterSegmentTask task = new PngEncoderDeflaterSegmentTask(originalSegment, deflatedSegment, compressionLevel, compressionStrategy, lastSegment);
        submitTask(task);
    ...
    void submitTask(PngEncoderDeflaterSegmentTask task) {
        ...
        resultQueue.offer(task);
    }

    void joinOne() throws IOException {
      PngEncoderDeflaterSegmentTask resultFuture = resultQueue.poll();
        ...
        if (resultFuture != null) {
            ...
            try {
                ...
                throw new IOException("An async segment task failed.");
