...
public final class LzoRawCompressor
{
    ...
    private static final int MAX_DISTANCE = 0xC000 - 1;
            ...
            final int[] table)
    {
        ...
        table[hash(UnsafeUtils.getLong(inputBase, input), mask)] = (int) (input - inputAddress);

        input++;
        int nextHash = hash(UnsafeUtils.getLong(inputBase, input), mask);
        ...
        do {
            ...
            do {
                ...
                nextHash = hash(UnsafeUtils.getLong(inputBase, nextInputIndex), mask);

                // put position on hash
                table[hash] = (int) (input - inputAddress);
            }
            while (UnsafeUtils.getInt(inputBase, matchIndex) != UnsafeUtils.getInt(inputBase, input) || matchIndex + MAX_DISTANCE < input);

            // catch up
            while ((input > anchor) && (matchIndex > inputAddress) && (UnsafeUtils.getByte(inputBase, input - 1) == UnsafeUtils.getByte(inputBase, matchIndex - 1))) {
                --input;
                --matchIndex;
            }
            ...
            while (true) {
                ...
                table[hash(UnsafeUtils.getLong(inputBase, position), mask)] = (int) (position - inputAddress);

                // Test next position
                int hash = hash(UnsafeUtils.getLong(inputBase, input), mask);
                matchIndex = inputAddress + table[hash];
                table[hash] = (int) (input - inputAddress);

                if (matchIndex + MAX_DISTANCE < input || UnsafeUtils.getInt(inputBase, matchIndex) != UnsafeUtils.getInt(inputBase, input)) {
                    input++;
                    nextHash = hash(UnsafeUtils.getLong(inputBase, input), mask);
                    break;
                }
    ...
    private static int count(byte[] inputBase, final long start, long matchStart, long matchLimit)
    {
        ...
        while (current < matchLimit - (SIZE_OF_LONG - 1)) {
            long diff = UnsafeUtils.getLong(inputBase, matchStart) ^ UnsafeUtils.getLong(inputBase, current);
            if (diff != 0) {
        ...
        if (current < matchLimit - (SIZE_OF_INT - 1) && UnsafeUtils.getInt(inputBase, matchStart) == UnsafeUtils.getInt(inputBase, current)) {
            current += SIZE_OF_INT;
            matchStart += SIZE_OF_INT;
        }

        if (current < matchLimit - (SIZE_OF_SHORT - 1) && UnsafeUtils.getShort(inputBase, matchStart) == UnsafeUtils.getShort(inputBase, current)) {
            current += SIZE_OF_SHORT;
            matchStart += SIZE_OF_SHORT;
        }

        if (current < matchLimit && UnsafeUtils.getByte(inputBase, matchStart) == UnsafeUtils.getByte(inputBase, current)) {
            ++current;
        }
            ...
            final long literalLength)
    {
        ...
        UnsafeUtils.copyMemory(inputBase, inputAddress, outputBase, output, literalLength);
        output += literalLength;

        // write stop command
        // this is a 0b0001_HMMM command with a zero match offset
        UnsafeUtils.putByte(outputBase, output++, (byte) 0x11);
        UnsafeUtils.putShort(outputBase, output, (byte) 0);
            ...
            long length)
    {
        if (firstLiteral && length < (0xFF - 17)) {
            UnsafeUtils.putByte(outBase, output++, (byte) (length + 17));
        }
        else if (length < 4) {
            ...
            UnsafeUtils.putByte(outBase, output - 2, (byte) (UnsafeUtils.getByte(outBase, output - 2) | length));
        }
        else {
            length -= 3;
            if (length > RUN_MASK) {
                UnsafeUtils.putByte(outBase, output++, (byte) 0);
                ...
                UnsafeUtils.putByte(outBase, output++, (byte) remaining);
            }
            else {
                UnsafeUtils.putByte(outBase, output++, (byte) length);
            }
        }
        return output;
    }

    private static long emitCopy(byte[] outputBase, long output, int matchOffset, int matchLength)
    {
        ...
        if (matchLength <= 8 && matchOffset <= 2048) {
            ...
            UnsafeUtils.putByte(outputBase, output++, (byte) (((matchLength) << 5) | ((matchOffset & 0x7) << 2)));
            UnsafeUtils.putByte(outputBase, output++, (byte) (matchOffset >>> 3));
        ...
        if (matchOffset >= (1 << 15)) {
            ...
            output = encodeMatchLength(outputBase, output, matchLength, 0x7, 0x18);
        }
        else if (matchOffset > (1 << 14)) {
            // 0b0001_0MMM (0bMMMM_MMMM)* 0bPPPP_PPPP_PPPP_PPLL
            output = encodeMatchLength(outputBase, output, matchLength, 0x7, 0x10);
        }
        else {
            // 0b001M_MMMM (0bMMMM_MMMM)* 0bPPPP_PPPP_PPPP_PPLL
            output = encodeMatchLength(outputBase, output, matchLength, 0x1F, 0x20);
    ...
    private static long encodeOffset(final byte[] outputBase, final long outputAddress, final int offset)
    {
        UnsafeUtils.putShort(outputBase, outputAddress, (short) (offset << 2));
        return outputAddress + 2;
    }

    private static long encodeMatchLength(byte[] outputBase, long output, int matchLength, int baseMatchLength, int command)
    {
        if (matchLength <= baseMatchLength) {
            UnsafeUtils.putByte(outputBase, output++, (byte) (command | matchLength));
        }
        else {
            UnsafeUtils.putByte(outputBase, output++, (byte) command);
            long remaining = matchLength - baseMatchLength;
            while (remaining > 510) {
                UnsafeUtils.putShort(outputBase, output, (short) 0);
                output += SIZE_OF_SHORT;
                remaining -= 510;
            }
            if (remaining > 255) {
                UnsafeUtils.putByte(outputBase, output++, (byte) 0);
                remaining -= 255;
            }
            UnsafeUtils.putByte(outputBase, output++, (byte) remaining);
        }
        return output;
    }
