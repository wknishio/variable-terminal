...
import static org.vash.vate.io.airlift.compress.zstd.FiniteStateEntropy.MAX_SYMBOL;
import static org.vash.vate.io.airlift.compress.zstd.FiniteStateEntropy.MIN_TABLE_LOG;
...
class FseTableReader
{
    ...
    public int readFseTable(FiniteStateEntropy.Table table, Object inputBase, long inputAddress, long inputLimit, int maxSymbol, int maxTableLog)
    {
        ...
        int bitStream = UnsafeUtils.getInt(inputBase, input);
        ...
        while (remaining > 1 && symbolNumber <= maxSymbol) {
            if (previousIsZero) {
                ...
                while ((bitStream & 0xFFFF) == 0xFFFF) {
                    ...
                    if (input < inputLimit - 5) {
                        input += 2;
                        bitStream = (UnsafeUtils.getInt(inputBase, input) >>> bitCount);
                    }
                    else {
                        // end of bit stream
                        bitStream >>>= 16;
            ...
            bitStream = UnsafeUtils.getInt(inputBase, input) >>> (bitCount & 31);
        }
