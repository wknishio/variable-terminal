...
import static org.vash.vate.io.airlift.compress.zstd.BitInputStream.isEndOfStream;
import static org.vash.vate.io.airlift.compress.zstd.BitInputStream.peekBitsFast;
...
import static org.vash.vate.io.airlift.compress.zstd.ZstdUtil.isPowerOf2;
...
class Huffman
{
    ...
    public int readTable(final Object inputBase, final long inputAddress, final int size)
    {
        ...
        int inputSize = UnsafeUtils.getByte(inputBase, input++) & 0xFF;
        ...
        if (inputSize >= 128) {
            ...
            for (int i = 0; i < outputSize; i += 2) {
                int value = UnsafeUtils.getByte(inputBase, input + i / 2) & 0xFF;
    ...
    public void decode4Streams(final Object inputBase, final long inputAddress, final long inputLimit, final Object outputBase, final long outputAddress, final long outputLimit)
    {
        ...
        long start2 = start1 + (UnsafeUtils.getShort(inputBase, inputAddress) & 0xFFFF);
        long start3 = start2 + (UnsafeUtils.getShort(inputBase, inputAddress + 2) & 0xFFFF);
        long start4 = start3 + (UnsafeUtils.getShort(inputBase, inputAddress + 4) & 0xFFFF);
