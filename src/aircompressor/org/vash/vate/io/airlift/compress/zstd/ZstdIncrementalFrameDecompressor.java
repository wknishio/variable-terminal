...
import static org.vash.vate.io.airlift.compress.zstd.ZstdFrameDecompressor.MAX_WINDOW_SIZE;
import static org.vash.vate.io.airlift.compress.zstd.ZstdFrameDecompressor.decodeRawBlock;
import static org.vash.vate.io.airlift.compress.zstd.ZstdFrameDecompressor.decodeRleBlock;
import static org.vash.vate.io.airlift.compress.zstd.ZstdFrameDecompressor.readFrameHeader;
import static org.vash.vate.io.airlift.compress.zstd.ZstdFrameDecompressor.verifyMagic;
import static org.vash.vate.io.airlift.compress.zstd.ZstdUtil.checkArgument;
...
public class ZstdIncrementalFrameDecompressor
{
        ...
        while (true) {
            // Flush ready output
            {
                ...
                if (flushableOutputSize > 0) {
                    ...
                    if (freeOutputSize > 0) {
                        ...
                        System.arraycopy(windowBase, (int)(windowAddress - 0), outputArray, output, copySize);
                        if (partialHash != null) {
            ...
            if (state == State.READ_BLOCK_HEADER) {
                ...
                if (inputBufferSize >= SIZE_OF_INT) {
                    blockHeader = UnsafeUtils.getInt(inputBase, input) & 0xFFFFFF;
                }
                else {
                    blockHeader = UnsafeUtils.getByte(inputBase, input) & 0xFF |
                            (UnsafeUtils.getByte(inputBase, input + 1) & 0xFF) << 8 |
                            (UnsafeUtils.getByte(inputBase, input + 2) & 0xFF) << 16;
                    int expected = UnsafeUtils.getInt(inputBase, input) & 0xFFFFFF;
            ...
            if (state == State.READ_BLOCK) {
                ...
                int blockSize = (blockHeader >>> 3) & 0x1FFFFF; // 21 bits
            ...
            if (state == State.READ_BLOCK_CHECKSUM) {
                if (frameHeader.hasChecksum) {
                    ...
                    int pendingOutputSize = (int)(windowPosition - windowAddress);
                    partialHash.update(windowBase, (int)(windowAddress - 0), pendingOutputSize);
    ...
    {
        return max(0, (int)(windowPosition - windowAddress - (frameHeader == null ? 0 : frameHeader.computeRequiredOutputBufferLookBackSize())));
    }
    ...
    private void resizeWindowBufferIfNecessary(FrameHeader frameHeader, int blockType, int blockSize)
    {
        ...
        if (windowLimit - windowPosition < MAX_BLOCK_SIZE) {
            ...
            int windowContentsSize = (int)(windowPosition - windowAddress);
            ...
            if (windowAddress != 0) {
                ...
                System.arraycopy(windowBase, (int)(windowAddress - 0), windowBase, 0, windowContentsSize);
                windowAddress = 0;
            ...
            if (windowLimit - windowPosition < maxBlockOutput) {
                ...
                if (frameHeader.contentSize >= 0 && frameHeader.contentSize < requiredWindowSize) {
                    newWindowSize = (int)(frameHeader.contentSize);
                }
                else {
    ...
    private static int determineFrameHeaderSize(final Object inputBase, final long inputAddress, final long inputLimit)
    {
        ...
        int frameHeaderDescriptor = UnsafeUtils.getByte(inputBase, inputAddress) & 0xFF;
