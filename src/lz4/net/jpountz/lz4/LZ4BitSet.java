 ...
 * of the bit set has a {@code boolean} value. The bits of a {@code LZ4BitSet}
 * are indexed by nonnegative integers. Individual indexed bits can be examined,
 * set, or cleared. One {@code LZ4BitSet} may be used to modify the contents of
 * another {@code LZ4BitSet} through logical AND, logical inclusive OR, and
 ...
 * {@code LZ4BitSet} will result in a {@code NullPointerException}.
 *
 * <p>
 * A {@code LZ4BitSet} is not safe for multithreaded use without external
 * synchronization.
 *
 ...
 * @since JDK1.0
 */
public class LZ4BitSet extends BitSet implements Cloneable, java.io.Serializable {
	/*
	 * LZ4BitSets are packed into arrays of "words." Currently a word is a long,
	 * which consists of 64 bits, requiring 6 address bits. The choice of word size
	 ...
	 *                  long[]
	 *
	 *                  The bits in this LZ4BitSet. The ith bit is stored in
	...
	/**
	 * The number of words in the logical size of this LZ4BitSet.
	 */
	...
	private void recalculateWordsInUse() {
		// Traverse the LZ4BitSet until a used word is found
		int i;
	 ...
	 * <p>
	 * More precisely, <br>
	 * {@code LZ4BitSet.valueOf(longs).get(n) == ((longs[n/64] & (1L<<(n%64))) != 0)}
	 * <br>
	 * for all {@code n < 64 * longs.length}.
	 *
	 * <p>
	 * This method is equivalent to
	 * {@code LZ4BitSet.valueOf(LongBuffer.wrap(longs))}.
	 *
	 * @param longs
	 *            a long array containing a little-endian representation of a
	 *            sequence of bits to be used as the initial bits of the new bit set
	 * @return a {@code LZ4BitSet} containing all the bits in the long array
	 * @since 1.7
	 */
	public static LZ4BitSet valueOf(long[] longs) {
		int n;
		for (n = longs.length; n > 0 && longs[n - 1] == 0; n--)
			;
		return new LZ4BitSet(copyOf(longs, n));
	}
	 ...
	 * <p>
	 * More precisely, <br>
	 * {@code LZ4BitSet.valueOf(lb).get(n) == ((lb.get(lb.position()+n/64) & (1L<<(n%64))) != 0)}
	 * <br>
	 ...
	 * @return a {@code LZ4BitSet} containing all the bits in the buffer in the
	 *         specified range
	 * @since 1.7
	 */
	public static LZ4BitSet valueOf(LongBuffer lb) {
		lb = lb.slice();
		int n;
	 ...
	 * <p>
	 * More precisely, <br>
	 * {@code LZ4BitSet.valueOf(bytes).get(n) == ((bytes[n/8] & (1<<(n%8))) != 0)}
	 * <br>
	 * for all {@code n <  8 * bytes.length}.
	 *
	 * <p>
	 * This method is equivalent to
	 * {@code LZ4BitSet.valueOf(ByteBuffer.wrap(bytes))}.
	 *
	 * @param bytes
	 *            a byte array containing a little-endian representation of a
	 *            sequence of bits to be used as the initial bits of the new bit set
	 * @return a {@code LZ4BitSet} containing all the bits in the byte array
	 * @since 1.7
	 */
	public static LZ4BitSet valueOf(byte[] bytes) {
		return LZ4BitSet.valueOf(ByteBuffer.wrap(bytes));
	}
	 ...
	 * <p>
	 * More precisely, <br>
	 * {@code LZ4BitSet.valueOf(bb).get(n) == ((bb.get(bb.position()+n/8) & (1<<(n%8))) != 0)}
	 * <br>
	 ...
	 * @since 1.7
	 */
	public static LZ4BitSet valueOf(ByteBuffer bb) {
	...
	/**
	 * Ensures that the LZ4BitSet can hold enough words.
	 * 
	 * @param wordsRequired
	...
	/**
	 * Ensures that the LZ4BitSet can accommodate a given wordIndex, temporarily
	...
	/**
	 * Sets all of the bits in this LZ4BitSet to {@code false}.
	 *
	 * @since 1.4
	 */
	public void clear() {
	 ...
	 * this {@code LZ4BitSet}; otherwise, the result is {@code false}.
	 *
	 * @param bitIndex
	 *            the bit index
	...
	/**
	 * Returns a new {@code LZ4BitSet} composed of bits from this {@code LZ4BitSet}
	 ...
	 * @return a new {@code LZ4BitSet} from a range of this {@code LZ4BitSet}
	 * @throws IndexOutOfBoundsException
	 *             if {@code fromIndex} is negative, or {@code toIndex} is negative,
	 *             or {@code fromIndex} is larger than {@code toIndex}
	 * @since 1.4
	 */
	public LZ4BitSet get(int fromIndex, int toIndex) {
		checkRange(fromIndex, toIndex);
		...
		// If no set bits in range return empty LZ4BitSet
		if (len <= fromIndex || fromIndex == toIndex)
			return new LZ4BitSet(0);

		// An optimization
		if (toIndex > len)
			toIndex = len;

		LZ4BitSet result = new LZ4BitSet(toIndex - fromIndex);
	 ...
	 *
	 * <p>
	 * To iterate over the {@code true} bits in a {@code LZ4BitSet}, use the
	 * following loop:
	 *
	 * <pre>
	 *  {@code
	...
	public int nextClearBit(int fromIndex) {
		// Neither spec nor implementation handle LZ4BitSets of maximal length.
		// See 4816253.
		if (fromIndex < 0)
			throw new IndexOutOfBoundsException("fromIndex < 0: " + fromIndex);
	...
	/**
	 * Returns the "logical size" of this {@code LZ4BitSet}: the index of the
	 * highest set bit in the {@code LZ4BitSet} plus one. Returns zero if the
	 * {@code LZ4BitSet} contains no set bits.
	 *
	 * @return the logical size of this {@code LZ4BitSet}
	 * @since 1.2
	 */
	public int length() {
		if (wordsInUse == 0)
	...
	/**
	 * Returns true if this {@code LZ4BitSet} contains no bits that are set to
	 * {@code true}.
	 *
	 * @return boolean indicating whether this {@code LZ4BitSet} is empty
	 * @since 1.4
	 */
	...
	/**
	 * Returns true if the specified {@code LZ4BitSet} has any bits set to
	 * {@code true} that are also set to {@code true} in this {@code LZ4BitSet}.
	 *
	 * @param set
	 *            {@code LZ4BitSet} to intersect with
	 * @return boolean indicating whether this {@code LZ4BitSet} intersects the
	 *         specified {@code LZ4BitSet}
	 * @since 1.4
	 */
	public boolean intersects(LZ4BitSet set) {
	...
	/**
	 * Returns the number of bits set to {@code true} in this {@code LZ4BitSet}.
	 *
	 * @return the number of bits set to {@code true} in this {@code LZ4BitSet}
	 * @since 1.4
	 */
	...
	/**
	 * Clears all of the bits in this {@code LZ4BitSet} whose corresponding bit is
	 * set in the specified {@code LZ4BitSet}.
	 *
	 * @param set
	 *            the {@code LZ4BitSet} with which to mask this {@code LZ4BitSet}
	 * @since 1.2
	 */
	public void andNot(LZ4BitSet set) {
	 ...
	 * which bits are set within this {@code LZ4BitSet}.
	 *
	 * <p>
	...
	/**
	 * Returns the number of bits of space actually in use by this {@code LZ4BitSet}
	 ...
	 * if and only if the argument is not {@code null} and is a {@code LZ4BitSet}
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof LZ4BitSet))
			return false;
		if (this == obj)
			return true;

		LZ4BitSet set = (LZ4BitSet) obj;
		...
		// Check words in use by both LZ4BitSets
		for (int i = 0; i < wordsInUse; i++)
	...
	/**
	 * Cloning this {@code LZ4BitSet} produces a new {@code LZ4BitSet} that is equal
	 * to it. The clone of the bit set is another bit set that has exactly the same
	...
	public Object clone() {
		...
		try {
			LZ4BitSet result = (LZ4BitSet) super.clone();
	...
	/**
	 * Save the state of the {@code LZ4BitSet} instance to a stream (i.e., serialize
	 * it).
	 */
	...
	/**
	 * Reconstitute the {@code LZ4BitSet} instance from a stream (i.e., deserialize
	 * it).
	 */
	 ...
	 * this {@code LZ4BitSet} contains a bit in the set state, the decimal
	 ...
	 * 
	 * <pre>
	 * LZ4BitSet drPepper = new LZ4BitSet();
	 * </pre>
	 * 
