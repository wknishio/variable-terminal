...
public abstract class LZ4Compressor {
	 ...
	 * length.
	 *
	 * This method will throw a {@link LZ4Exception} if this compressor is unable to
	 * compress the input into less than <code>maxDestLen</code> bytes. To prevent
	 * this exception to be thrown, you should make sure that
	 * <code>maxDestLen &gt;= maxCompressedLength(srcLen)</code>.
	 *
	 * @param src
	 *            the source data
	 * @param srcOff
	 *            the start offset in src
	 * @param srcLen
	 ...
	 * @param maxDestLen
	 *            the maximum number of bytes to write in dest
	 * @throws LZ4Exception
	 ...
	 * {@link #compress(byte[], int, int, byte[], int, int) compress(src, srcOff,
	 * srcLen, dest, destOff, dest.length - destOff)}.
	 *
	 * @param src
	 *            the source data
	 * @param srcOff
	...
	/**
	 * Convenience method, equivalent to calling
	 * {@link #compress(byte[], int, int, byte[], int) compress(src, 0, src.length,
	 * dest, 0)}.
	 *
	 * @param src
	 *            the source data
	 ...
	 * compressed.
	 * <p>
	 * <b><span style="color:red">Warning</span></b>: this method has an important
	 * overhead due to the fact that it needs to allocate a buffer to compress into,
	 * and then needs to resize this buffer to the actual compressed length.
	 * </p>
	 * <p>
	...
	/**
	 * Convenience method, equivalent to calling {@link #compress(byte[], int, int)
	...
	/**
	 * Compresses <code>src</code> into <code>dest</code>. Calling this method will
	 * update the positions of both {@link ByteBuffer}s.
	 *
	 * @param src
	 *            the source data
	 * @param dest
