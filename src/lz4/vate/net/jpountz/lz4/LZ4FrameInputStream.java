...
public class LZ4FrameInputStream extends FilterInputStream {
	...
	/**
	 * Creates a new {@link InputStream} that will decompress data using fastest
	 * instances of {@link LZ4SafeDecompressor} and {@link XXHash32}.
	 *
	 * @param in
	...
	/**
	 * Creates a new {@link InputStream} that will decompress data using the LZ4
	 * algorithm.
	 *
	 * @param in
	 *            the stream to decompress
	...
	/**
	 * Try and load in the next valid frame info. This will skip over skippable
	 * frames.
	 * 
	 * @return True if a frame was loaded. False if there are no more frames in the
	 *         stream.
	 * @throws IOException
	...
	private final ByteBuffer readNumberBuff = ByteBuffer.allocate(LZ4FrameOutputStream.LONG_BYTES)
	...
	/**
	 * Decompress (if necessary) buffered data, optionally computes and validates a
	 * XXHash32 checksum, and writes the result to a buffer.
	 *
	 * @throws IOException
	 */
	private void readBlock() throws IOException {
		...
		if (blockSize > maxBlockSize) {
			throw new IOException(
					String.format(Locale.getDefault(), "Block size %s exceeded max: %s", blockSize, maxBlockSize));
		}
