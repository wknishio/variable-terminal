 ...
 */
package vate.com.offbynull.portmapper.helpers;
...
//            offset += component4.length();
    ...
    private static String readIp6(String text, int offset) {
        try {
            List<String> components = new ArrayList<String>(IPV6_MAX_NUMBER_OF_COMPONENTS);
            ...
            if (components.size() < IPV6_MAX_NUMBER_OF_COMPONENTS) {
                ...
                if (components.size() == 3
                        && components.get(0).length() <= 0
                        && components.get(1).length() <= 0
                        && components.get(2).length() <= 0) {
                    // SPECIAL CASE:
                    ...
                    Validate.isTrue(2 == getEmptyComponentCount(components)); // already made sure there's 2 in the start, making sure there
                        ...
                        && components.get(components.size() - 2).length() <= 0
                        && components.get(components.size() - 1).length() <= 0) {
                    // SPECIAL CASE:
