 ...
 * {@link LZ4FastDecompressor} but do not require the size of the original data
 * to be known.
 */
public abstract class LZ4SafeDecompressor implements LZ4UnknownSizeDecompressor {
	 ...
	 *             if maxDestLen is too small
	 */
	public abstract int decompress(ByteBuffer src, int srcOff, int srcLen, ByteBuffer dest, int destOff,
			int maxDestLen);

	/**
	 * Convenience method, equivalent to calling
	 * {@link #decompress(byte[], int, int, byte[], int, int) decompress(src,
	 * srcOff, srcLen, dest, destOff, dest.length - destOff)}.
	 *
	 * @param src
	 *            the compressed data
	 * @param srcOff
	...
	/**
	 * Convenience method, equivalent to calling
	 * {@link #decompress(byte[], int, int, byte[], int) decompress(src, 0,
	 * src.length, dest, 0)}
	 *
	 * @param src
	 *            the compressed data
	 ...
	 * overhead due to the fact that it needs to allocate a buffer to decompress
	 * into, and then needs to resize this buffer to the actual decompressed length.
	 * </p>
	 * <p>
	...
	/**
	 * Convenience method, equivalent to calling
	 * {@link #decompress(byte[], int, int, int) decompress(src, 0, src.length,
	 * maxDestLen)}.
	 *
	 * @param src
	 *            the compressed data
	...
	public final void decompress(ByteBuffer src, ByteBuffer dest) {
		final int decompressed = decompress(src, src.position(), src.remaining(), dest, dest.position(),
				dest.remaining());
