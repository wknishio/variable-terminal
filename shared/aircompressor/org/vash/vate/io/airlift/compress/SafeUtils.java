 ...
 *******************************************************************************/
package org.vash.vate.io.airlift.compress;
...
public enum SafeUtils {
	...
	public static int readShort(byte[] buf, int off) {
    return readShortLE(buf, off);
  }
	
	public static void writeShort(byte[] buf, int off, int v) {
    writeShortLE(buf, off, v);
  }
	
	public static long readLong(byte[] buf, int i) {
	   return readLongLE(buf, i);
	}
	
	 public static void writeIntLE(byte[] buf, int off, int v) {
	    buf[off++] = (byte) v;
	    buf[off++] = (byte) (v >>> 8);
	    buf[off++] = (byte) (v >>> 16);
	    buf[off++] = (byte) (v >>> 24);
	...
	public static void writeLongLE(byte[] buf, int off, long v) {
	  buf[off++] = (byte) v;
    buf[off++] = (byte) (v >>> 8);
    buf[off++] = (byte) (v >>> 16);
    buf[off++] = (byte) (v >>> 24);
