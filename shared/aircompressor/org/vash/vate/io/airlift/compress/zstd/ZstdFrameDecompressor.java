...
import static org.vash.vate.io.airlift.compress.zstd.BitInputStream.peekBits;
import static org.vash.vate.io.airlift.compress.zstd.Constants.COMPRESSED_BLOCK;
import static org.vash.vate.io.airlift.compress.zstd.Constants.COMPRESSED_LITERALS_BLOCK;
import static org.vash.vate.io.airlift.compress.zstd.Constants.DEFAULT_MAX_OFFSET_CODE_SYMBOL;
import static org.vash.vate.io.airlift.compress.zstd.Constants.LITERALS_LENGTH_BITS;
import static org.vash.vate.io.airlift.compress.zstd.Constants.LITERAL_LENGTH_TABLE_LOG;
import static org.vash.vate.io.airlift.compress.zstd.Constants.LONG_NUMBER_OF_SEQUENCES;
import static org.vash.vate.io.airlift.compress.zstd.Constants.MAGIC_NUMBER;
import static org.vash.vate.io.airlift.compress.zstd.Constants.MATCH_LENGTH_BITS;
import static org.vash.vate.io.airlift.compress.zstd.Constants.MATCH_LENGTH_TABLE_LOG;
import static org.vash.vate.io.airlift.compress.zstd.Constants.MAX_BLOCK_SIZE;
import static org.vash.vate.io.airlift.compress.zstd.Constants.MAX_LITERALS_LENGTH_SYMBOL;
import static org.vash.vate.io.airlift.compress.zstd.Constants.MAX_MATCH_LENGTH_SYMBOL;
import static org.vash.vate.io.airlift.compress.zstd.Constants.MIN_BLOCK_SIZE;
import static org.vash.vate.io.airlift.compress.zstd.Constants.MIN_SEQUENCES_SIZE;
import static org.vash.vate.io.airlift.compress.zstd.Constants.MIN_WINDOW_LOG;
import static org.vash.vate.io.airlift.compress.zstd.Constants.OFFSET_TABLE_LOG;
import static org.vash.vate.io.airlift.compress.zstd.Constants.RAW_BLOCK;
import static org.vash.vate.io.airlift.compress.zstd.Constants.RAW_LITERALS_BLOCK;
import static org.vash.vate.io.airlift.compress.zstd.Constants.RLE_BLOCK;
import static org.vash.vate.io.airlift.compress.zstd.Constants.RLE_LITERALS_BLOCK;
import static org.vash.vate.io.airlift.compress.zstd.Constants.SEQUENCE_ENCODING_BASIC;
import static org.vash.vate.io.airlift.compress.zstd.Constants.SEQUENCE_ENCODING_COMPRESSED;
import static org.vash.vate.io.airlift.compress.zstd.Constants.SEQUENCE_ENCODING_REPEAT;
import static org.vash.vate.io.airlift.compress.zstd.Constants.SEQUENCE_ENCODING_RLE;
import static org.vash.vate.io.airlift.compress.zstd.Constants.SIZE_OF_BLOCK_HEADER;
import static org.vash.vate.io.airlift.compress.zstd.Constants.SIZE_OF_BYTE;
import static org.vash.vate.io.airlift.compress.zstd.Constants.SIZE_OF_INT;
import static org.vash.vate.io.airlift.compress.zstd.Constants.SIZE_OF_LONG;
import static org.vash.vate.io.airlift.compress.zstd.Constants.SIZE_OF_SHORT;
import static org.vash.vate.io.airlift.compress.zstd.Constants.TREELESS_LITERALS_BLOCK;
import static org.vash.vate.io.airlift.compress.zstd.ZstdUtil.fail;
import static org.vash.vate.io.airlift.compress.zstd.ZstdUtil.get24BitLittleEndian;
import static org.vash.vate.io.airlift.compress.zstd.ZstdUtil.mask;
import static org.vash.vate.io.airlift.compress.zstd.ZstdUtil.verify;

import org.vash.vate.compatibility.VTArrays;
import org.vash.vate.io.airlift.compress.MalformedInputException;
import org.vash.vate.io.airlift.compress.UnsafeUtils;
...
class ZstdFrameDecompressor
{
            ...
            final long outputLimit)
    {
        ...
        while (input < inputLimit) {
            ...
            do {
                ...
                int blockType = (header >>> 1) & 0x3;
                int blockSize = (header >>> 3) & 0x1FFFFF; // 21 bits
            ...
            if (frameHeader.hasChecksum) {
                ...
                int checksum = UnsafeUtils.getInt(inputBase, input);
    ...
    static int decodeRawBlock(Object inputBase, long inputAddress, int blockSize, Object outputBase, long outputAddress, long outputLimit)
    {
        ...
        UnsafeUtils.copyMemory(inputBase, inputAddress, outputBase, outputAddress, blockSize);
        return blockSize;
    }
    ...
    static int decodeRleBlock(int size, Object inputBase, long inputAddress, Object outputBase, long outputAddress, long outputLimit)
    {
        ...
        long output = outputAddress;
        long value = UnsafeUtils.getByte(inputBase, inputAddress) & 0xFFL;
        ...
        if (remaining >= SIZE_OF_LONG) {
            ...
            do {
              UnsafeUtils.putLong(outputBase, output, packed);
                output += SIZE_OF_LONG;
        ...
        for (int i = 0; i < remaining; i++) {
          UnsafeUtils.putByte(outputBase, output, (byte) value);
            output++;
        }
            ...
            long outputAbsoluteBaseAddress)
    {
        ...
        // decode literals
        int literalsBlockType = UnsafeUtils.getByte(inputBase, input) & 0x3;
            ...
            long outputAbsoluteBaseAddress)
    {
        ...
        // decode header
        int sequenceCount = UnsafeUtils.getByte(inputBase, input++) & 0xFF;
        if (sequenceCount != 0) {
            if (sequenceCount == 255) {
                verify(input + SIZE_OF_SHORT <= inputLimit, input, "Not enough input bytes");
                sequenceCount = (UnsafeUtils.getShort(inputBase, input) & 0xFFFF) + LONG_NUMBER_OF_SEQUENCES;
                input += SIZE_OF_SHORT;
            }
            else if (sequenceCount > 127) {
                verify(input < inputLimit, input, "Not enough input bytes");
                sequenceCount = ((sequenceCount - 128) << 8) + (UnsafeUtils.getByte(inputBase, input++) & 0xFF);
            }

            verify(input + SIZE_OF_INT <= inputLimit, input, "Not enough input bytes");

            byte type = UnsafeUtils.getByte(inputBase, input++);

            int literalsLengthType = (type & 0xFF) >>> 6;
            int offsetCodesType = (type >>> 4) & 0x3;
            int matchLengthType = (type >>> 2) & 0x3;
    ...
    private static long copyLastLiteral(Object outputBase, Object literalsBase, long literalsLimit, long output, long literalsInput)
    {
        ...
        UnsafeUtils.copyMemory(literalsBase, literalsInput, outputBase, output, lastLiteralsSize);
    ...
    private static long copyMatchHead(Object outputBase, long output, int offset, long matchAddress)
    {
        ...
        if (offset < 8) {
            ...
            UnsafeUtils.putInt(outputBase, output + 4, UnsafeUtils.getInt(outputBase, matchAddress));
    ...
    private static long copyLiterals(Object outputBase, Object literalsBase, long output, long literalsInput, long literalOutputLimit)
    {
        ...
        do {
          UnsafeUtils.putLong(outputBase, output, UnsafeUtils.getLong(literalsBase, literalInput));
            output += SIZE_OF_LONG;
    ...
    private void executeLastSequence(Object outputBase, long output, long literalOutputLimit, long matchOutputLimit, long fastOutputLimit, long literalInput, long matchAddress)
    {
        ...
        while (output < literalOutputLimit) {
          UnsafeUtils.putByte(outputBase, output, UnsafeUtils.getByte(literalsBase, literalInput));
            output++;
            literalInput++;
        }
    ...
    private int decodeCompressedLiterals(Object inputBase, final long inputAddress, int blockSize, int literalsBlockType)
    {
        ...
        int headerSize;
        int type = (UnsafeUtils.getByte(inputBase, input) >> 2) & 0x3;
        switch (type) {
            case 0:
                singleStream = true;
            case 1: {
                int header = UnsafeUtils.getInt(inputBase, input);
            ...
            case 3: {
                ...
                long header = UnsafeUtils.getByte(inputBase, input) & 0xFF |
                        (UnsafeUtils.getInt(inputBase, input + 1) & 0xFFFFFFFFL) << 8;
    ...
    private int decodeRleLiterals(Object inputBase, final long inputAddress, int blockSize)
    {
        ...
        switch (type) {
            case 0:
            case 2:
                outputSize = (UnsafeUtils.getByte(inputBase, input) & 0xFF) >>> 3;
                input++;
                break;
            case 1:
                outputSize = (UnsafeUtils.getShort(inputBase, input) & 0xFFFF) >>> 4;
                input += 2;
                break;
            case 3:
                // we need at least 4 bytes (3 for the header, 1 for the payload)
                verify(blockSize >= SIZE_OF_INT, input, "Not enough input bytes");
                outputSize = (UnsafeUtils.getInt(inputBase, input) & 0xFFFFFF) >>> 4;
                input += 3;
                break;
            default:
    ...
    private int decodeRawLiterals(Object inputBase, final long inputAddress, long inputLimit)
    {
        ...
        switch (type) {
            case 0:
            case 2:
                literalSize = (UnsafeUtils.getByte(inputBase, input) & 0xFF) >>> 3;
                input++;
                break;
            case 1:
                literalSize = (UnsafeUtils.getShort(inputBase, input) & 0xFFFF) >>> 4;
                input += 2;
                break;
            case 3:
                // read 3 little-endian bytes
                int header = ((UnsafeUtils.getByte(inputBase, input) & 0xFF) |
                        ((UnsafeUtils.getShort(inputBase, input + 1) & 0xFFFF) << 8));
    ...
    static FrameHeader readFrameHeader(final Object inputBase, final long inputAddress, final long inputLimit)
    {
        ...
        int frameHeaderDescriptor = UnsafeUtils.getByte(inputBase, input++) & 0xFF;
        boolean singleSegment = (frameHeaderDescriptor & 0x20) != 0;
        int dictionaryDescriptor = frameHeaderDescriptor & 0x3;
        ...
        int windowSize = -1;
        if (!singleSegment) {
            int windowDescriptor = UnsafeUtils.getByte(inputBase, input++) & 0xFF;
            int exponent = windowDescriptor >>> 3;
            int mantissa = windowDescriptor & 0x7;
        ...
        switch (dictionaryDescriptor) {
            case 1:
                dictionaryId = UnsafeUtils.getByte(inputBase, input) & 0xFF;
                input += SIZE_OF_BYTE;
                break;
            case 2:
                dictionaryId = UnsafeUtils.getShort(inputBase, input) & 0xFFFF;
                input += SIZE_OF_SHORT;
                break;
            case 3:
                dictionaryId = UnsafeUtils.getInt(inputBase, input) & 0xFFFFFFFFL;
                input += SIZE_OF_INT;
                break;
        }
        ...
        switch (contentSizeDescriptor) {
            case 0:
                if (singleSegment) {
                    contentSize = UnsafeUtils.getByte(inputBase, input) & 0xFF;
                    input += SIZE_OF_BYTE;
                }
                break;
            case 1:
                contentSize = UnsafeUtils.getShort(inputBase, input) & 0xFFFF;
                contentSize += 256;
                input += SIZE_OF_SHORT;
                break;
            case 2:
                contentSize = UnsafeUtils.getInt(inputBase, input) & 0xFFFFFFFFL;
                input += SIZE_OF_INT;
                break;
            case 3:
                contentSize = UnsafeUtils.getLong(inputBase, input);
                input += SIZE_OF_LONG;
                break;
        }

        boolean hasChecksum = (frameHeaderDescriptor & 0x4) != 0;
    ...
    static int verifyMagic(Object inputBase, long inputAddress, long inputLimit)
    {
        ...
        int magic = UnsafeUtils.getInt(inputBase, inputAddress);
