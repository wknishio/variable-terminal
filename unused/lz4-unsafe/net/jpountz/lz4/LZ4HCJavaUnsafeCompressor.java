...
import static org.vash.vate.net.jpountz.lz4.LZ4Utils.*;

import java.nio.ByteBuffer;
import java.util.Arrays;

import org.vash.vate.net.jpountz.lz4.LZ4Utils.Match;
...
final class LZ4HCJavaUnsafeCompressor extends LZ4Compressor {
	...
	private class HashTable {
		...
		boolean insertAndFindBestMatch(byte[] buf, int off, int matchLimit, Match match) {
			...
			if (ref >= off - 4 && ref <= off && ref >= base) { // potential repetition
				if (LZ4UnsafeUtils.readIntEquals(buf, ref, off)) { // confirmed
					delta = off - ref;
					repl = match.len = MIN_MATCH
							+ LZ4UnsafeUtils.commonBytes(buf, ref + MIN_MATCH, off + MIN_MATCH, matchLimit);
					match.ref = ref;
				}
				ref = next(ref);
			}
		...
		boolean insertAndFindWiderMatch(byte[] buf, int off, int startLimit, int matchLimit, int minLen, Match match) {
			...
			for (int i = 0; i < maxAttempts; ++i) {
				...
				if (LZ4UnsafeUtils.readIntEquals(buf, ref, off)) {
					final int matchLenForward = MIN_MATCH
							+ LZ4UnsafeUtils.commonBytes(buf, ref + MIN_MATCH, off + MIN_MATCH, matchLimit);
		...
		boolean insertAndFindBestMatch(ByteBuffer buf, int off, int matchLimit, Match match) {
			...
			if (ref >= off - 4 && ref <= off && ref >= base) { // potential repetition
				if (LZ4ByteBufferUtils.readIntEquals(buf, ref, off)) { // confirmed
					delta = off - ref;
					repl = match.len = MIN_MATCH
							+ LZ4ByteBufferUtils.commonBytes(buf, ref + MIN_MATCH, off + MIN_MATCH, matchLimit);
					match.ref = ref;
				}
				ref = next(ref);
			}
		...
		boolean insertAndFindWiderMatch(ByteBuffer buf, int off, int startLimit, int matchLimit, int minLen,
				Match match) {
			match.len = minLen;
			...
			for (int i = 0; i < maxAttempts; ++i) {
				...
				if (LZ4ByteBufferUtils.readIntEquals(buf, ref, off)) {
					final int matchLenForward = MIN_MATCH
							+ LZ4ByteBufferUtils.commonBytes(buf, ref + MIN_MATCH, off + MIN_MATCH, matchLimit);
					final int matchLenBackward = LZ4ByteBufferUtils.commonBytesBackward(buf, ref, off, base,
							startLimit);
					final int matchLen = matchLenBackward + matchLenForward;
					if (matchLen > match.len) {
						match.len = matchLen;
		...
		main: while (sOff < mfLimit) {
			...
			search2: while (true) {
				...
				if (match1.end() >= mfLimit || !ht.insertAndFindWiderMatch(src, match1.end() - 2, match1.start + 1,
					...
					// no better match
					dOff = LZ4UnsafeUtils.encodeSequence(src, anchor, match1.start, match1.ref, match1.len, dest, dOff,
							destEnd);
				...
				search3: while (true) {
					...
					if (match2.start + match2.len >= mfLimit || !ht.insertAndFindWiderMatch(src, match2.end() - 3,
						...
						// encode seq 1
						dOff = LZ4UnsafeUtils.encodeSequence(src, anchor, match1.start, match1.ref, match1.len, dest,
								dOff, destEnd);
						anchor = sOff = match1.end();
						// encode seq 2
						dOff = LZ4UnsafeUtils.encodeSequence(src, anchor, match2.start, match2.ref, match2.len, dest,
								dOff, destEnd);
					...
					if (match3.start < match1.end() + 3) { // Not enough space for match 2 : remove it
						if (match3.start >= match1.end()) { // // can write Seq1 immediately ==> Seq2 is removed, so
															// Seq3 becomes Seq1
							if (match2.start < match1.end()) {
							...
							dOff = LZ4UnsafeUtils.encodeSequence(src, anchor, match1.start, match1.ref, match1.len,
									dest, dOff, destEnd);
							anchor = sOff = match1.end();
	...
	public int compress(ByteBuffer src, int srcOff, int srcLen, ByteBuffer dest, int destOff, int maxDestLen) {
		...
		if (src.hasArray() && dest.hasArray()) {
			return compress(src.array(), srcOff + src.arrayOffset(), srcLen, dest.array(), destOff + dest.arrayOffset(),
					maxDestLen);
		}
		...
		main: while (sOff < mfLimit) {
			...
			search2: while (true) {
				...
				if (match1.end() >= mfLimit || !ht.insertAndFindWiderMatch(src, match1.end() - 2, match1.start + 1,
						matchLimit, match1.len, match2)) {
					// no better match
					dOff = LZ4ByteBufferUtils.encodeSequence(src, anchor, match1.start, match1.ref, match1.len, dest,
							dOff, destEnd);
					anchor = sOff = match1.end();
					continue main;
				}
				...
				search3: while (true) {
					...
					if (match2.start + match2.len >= mfLimit || !ht.insertAndFindWiderMatch(src, match2.end() - 3,
						...
						// encode seq 1
						dOff = LZ4ByteBufferUtils.encodeSequence(src, anchor, match1.start, match1.ref, match1.len,
								dest, dOff, destEnd);
						anchor = sOff = match1.end();
						// encode seq 2
						dOff = LZ4ByteBufferUtils.encodeSequence(src, anchor, match2.start, match2.ref, match2.len,
								dest, dOff, destEnd);
						anchor = sOff = match2.end();
						continue main;
					}
